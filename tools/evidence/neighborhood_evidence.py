"""Run and publish the external local-neighbor qualification matrix."""
...
def render_summary(run: EvidenceRun) -> str:
    ...
    lines = [
        "# BLITZAR Neighborhood Evidence",
        "",
        f"- status: `{status}`",
        f"- plan version: `{run.contract['plan_version']}`",
        f"- seed: `{run.contract['seed']}`",
        f"- records: `{len(run.records)}`",
        "- the Octree values are a separate long-range baseline, not a local-neighbor oracle",
        "- timing is wall-clock evidence; hardware counters are not claimed",
        "",
    ]
    if run.errors:
        ...
        lines.append("All declared local-neighbor candidates and scenarios passed.")
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.check:
            if contract_errors:
                for error in contract_errors:
                    print(f"neighborhood-evidence: {error}", file=sys.stderr)
                return 1
            print(f"neighborhood-evidence: contract {contract['plan_version']} is valid")
            return 0
        ...
        if not external_output(root, output):
            raise ValueError("neighborhood evidence output must be outside the source tree")
        ...
        if errors:
            for error in errors:
                print(f"neighborhood-evidence: {error}", file=sys.stderr)
            return 1
        print(f"neighborhood-evidence: {len(records)} records passed")
        return 0
