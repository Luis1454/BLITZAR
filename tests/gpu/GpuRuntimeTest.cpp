...
{
    return simulation.SetExecutionSettings(blitzar_core::ExecutionSettings::Fast()) ==
               BLITZAR_STATUS_OK &&
...
[[nodiscard]] bool RunStrictReferenceCase() noexcept
{
    const TestState state{};
    blitzar_sim::Sim simulation(TestParticleCount);
    ...
    return simulation.SetGravity(1.0, 0.1) == BLITZAR_STATUS_OK &&
           ...
           simulation.LastBackend() == BLITZAR_BACKEND_CPU;
}
...
int main()
{
    ...
    BLITZAR_CHECK(RunStrictReferenceCase());
