...
{
    if (const char *envPath = std::getenv("GRAVITY_BACKEND_EXE")) {
        const std::string fromEnv(envPath);
        if (!fromEnv.empty()) {
            return fromEnv;
        }
    }
    ...
    std::error_code ec;
    const std::string defaultName(sim::platform::backendDefaultExecutableName());
    const std::filesystem::path cwd = std::filesystem::current_path(ec);
    if (!ec) {
...
bool RealBackendHarness::isPortBindable(std::uint16_t port)
{
    ...
    const sim::socket::Handle handle = sim::socket::createTcpSocket();
    if (!sim::socket::isValid(handle)) {
        sim::socket::shutdownSocketLayer();
        return false;
    }

    (void)sim::socket::setReuseAddress(handle, true);
    const bool ok = sim::socket::bindIpv4(handle, "127.0.0.1", port);
    sim::socket::closeSocket(handle);
    sim::socket::shutdownSocketLayer();
    return ok;
}
