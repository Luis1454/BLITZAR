...
bool Initialize(blitzar_particles::ParticleBuffer& particles) noexcept
{
    ...
    const double circular_speed = std::sqrt(
        0.5 / std::pow(1.0 + softening * softening, 1.5));
    return particles.SetPosition(0, {-0.5, 0.0, 0.0}) == BLITZAR_STATUS_OK &&
           particles.SetPosition(1, {0.5, 0.0, 0.0}) == BLITZAR_STATUS_OK &&
           particles.SetVelocity(0, {0.0, circular_speed, 0.0}) ==
               BLITZAR_STATUS_OK &&
           particles.SetVelocity(1, {0.0, -circular_speed, 0.0}) ==
               BLITZAR_STATUS_OK &&
    ...
    double softening) noexcept
{
    ...
    for (std::size_t index = 0; index < state.count; ++index) {
        const double speed_squared =
            state.velocity_x[index] * state.velocity_x[index] +
    ...
    const double softened_distance = std::sqrt(
        dx * dx + dy * dy + dz * dz + softening * softening);
    const double potential =
        -gravitational_constant * state.mass[0] * state.mass[1] /
        softened_distance;
    return kinetic + potential;
}

bool SameState(
    blitzar_core::ParticleStateView first,
    blitzar_core::ParticleStateView second) noexcept
{
    for (std::size_t index = 0; index < first.count; ++index) {
        if (first.x[index] != second.x[index] ||
            first.y[index] != second.y[index] ||
...
int main()
{
    ...
    const double initial_energy = Energy(
        first_particles.State(), gravitational_constant, softening);
    ...
    const blitzar_core::ParticleStateView second_state =
    ...
    BLITZAR_CHECK(std::abs(
                      Energy(first_state, gravitational_constant, softening) -
                      initial_energy) <
                  1.0e-8);
    ...
    BLITZAR_CHECK(limit_particles.SetMass(
