...
TEST(ServerProtocolTest, TST_INT_PROT_011_FixedSeedServerReplayIsDeterministic)
{
    ...
    for (size_t i = 0; i < snapshotA.size(); ++i) {
        if (snapshotZeroA[i].x != snapshotA[i].x || 
            snapshotZeroA[i].y != snapshotA[i].y || 
    ...
    ServerClientResponse responseB = clientB.sendCommand(std::string(grav_protocol::Step), "\"count\":10");
    ...
    for (size_t i = 0; i < snapshotA.size(); ++i) {
        EXPECT_EQ(snapshotA[i].x, snapshotB[i].x) << "Mismatch at particle " << i << " X coordinate (drift)";
        EXPECT_EQ(snapshotA[i].y, snapshotB[i].y) << "Mismatch at particle " << i << " Y coordinate (drift)";
        EXPECT_EQ(snapshotA[i].z, snapshotB[i].z) << "Mismatch at particle " << i << " Z coordinate (drift)";
    }
}
