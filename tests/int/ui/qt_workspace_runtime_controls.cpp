...
class RecordingRuntime final : public grav_client::IClientRuntime {
    public:
        ...
        void stop() override {}
        void setPaused(bool paused) override { pausedState = paused; }
        void togglePaused() override { pausedState = !pausedState; }
        void stepOnce() override { stepCount += 1; }
        void setParticleCount(std::uint32_t particleCount) override { configuredParticleCount = particleCount; }
        void setDt(float dt) override { configuredDt = dt; }
        void scaleDt(float factor) override { configuredDt *= factor; }
        void requestReset() override { resetCount += 1; }
        void requestRecover() override { recoverCount += 1; }
        void setSolverMode(const std::string &mode) override { solverMode = mode; }
        void setIntegratorMode(const std::string &mode) override { integratorMode = mode; }
        void setPerformanceProfile(const std::string &profile) override { performanceProfile = profile; }
        void setOctreeParameters(float theta, float softening) override { octreeTheta = theta; octreeSoftening = softening; }
        void setSphEnabled(bool enabled) override { sphEnabled = enabled; }
        void setSphParameters(float h, float rho, float gasK, float visc) override { sphSmoothingLength = h; sphRestDensity = rho; sphGasConstant = gasK; sphViscosity = visc; }
        void setSubstepPolicy(float targetDt, std::uint32_t maxSubsteps) override { substepTargetDt = targetDt; configuredMaxSubsteps = maxSubsteps; }
        ...
        void configureRemoteConnector(const std::string &host, std::uint16_t port, bool autoStart, const std::string &serverExecutable) override { connectorHost = host; connectorPort = port; connectorAutoStart = autoStart; connectorServerExecutable = serverExecutable; }
        bool isRemoteMode() const override { return true; }
        ...
        std::string serverOwnerLabel() const override { return "external"; }
...
TEST(QtWorkspaceRuntimeControlsTest, TST_UIX_UI_011_RunAndSceneProfilesPropagateToRuntimeAndUi)
{
    ...
    QCoreApplication::processEvents(QEventLoop::AllEvents, 20);
    EXPECT_EQ(window.findChild<QComboBox *>("solverCombo")->currentText().toStdString(), "pairwise_cuda");
    EXPECT_EQ(window.findChild<QComboBox *>("integratorCombo")->currentText().toStdString(), "rk4");
    EXPECT_EQ(window.findChild<QComboBox *>("scenePresetCombo")->currentText().toStdString(), "two_body");
...
TEST(QtWorkspaceRuntimeControlsTest, TST_UIX_UI_012_PhysicsAndRenderControlsPersistWorkspaceChoices)
{
    ...
    const std::filesystem::path configPath = std::filesystem::temp_directory_path() / ("gravity_qt_workspace_runtime_" + std::to_string(stamp) + ".ini");
