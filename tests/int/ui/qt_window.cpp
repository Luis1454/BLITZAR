...
TEST(QtMainWindowTest, TST_UIX_UI_001_ConstructsAndTicksWithRealRuntime)
{
    ...
    auto runtime = std::make_unique<grav_client::ClientRuntime>("simulation.ini", testsupport::makeTransport(server.port(), server.executablePath()));
...
TEST(QtMainWindowTest, TST_UIX_UI_002_ShowsReconnectingWhenServerStopsAndRecovers)
{
    ...
    auto runtime = std::make_unique<grav_client::ClientRuntime>("simulation.ini", testsupport::makeTransport(fixedPort, server.executablePath()));
    grav_qt::MainWindow window(makeUiConfig(), "simulation.ini", std::move(runtime));
    ASSERT_TRUE(testsupport::waitUntilUi([&]() {
...
TEST(QtMainWindowTest, TST_UIX_UI_003_SavesConfigOnlyOnExplicitSaveAction)
{
    ...
    const std::filesystem::path configPath = std::filesystem::temp_directory_path() / ("gravity_qt_explicit_save_" + std::to_string(stamp) + ".ini");
    ...
    auto runtime = std::make_unique<grav_client::ClientRuntime>(configPath.string(), testsupport::makeTransport(server.port(), server.executablePath()));
    ...
    EXPECT_NE(fileBeforeSave.find("simulation(particle_count=10000, dt=0.01, solver=pairwise_cuda, integrator=euler)"), std::string::npos);

    QPushButton *saveButton = testsupport::findButtonByText(window, "Save config");
    ASSERT_NE(saveButton, nullptr);
    saveButton->click();
    ASSERT_TRUE(testsupport::waitUntilUi([&]() {
        return testsupport::readAllFile(configPath).find("simulation(particle_count=10000, dt=0.01, solver=octree_gpu, integrator=euler)") != std::string::npos;
