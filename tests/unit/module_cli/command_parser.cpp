...
{
    const grav_cmd::CommandParseResult parsed = grav_cmd::CommandParser::parseLine("load_config \"cfg#release.ini\" # trailing comment", 12u);
    ASSERT_TRUE(parsed.ok);
...
{
    const grav_cmd::CommandParseResult parsed = grav_cmd::CommandParser::parseLine("   # only comment", 2u);
    ASSERT_TRUE(parsed.ok);
...
{
    const grav_cmd::CommandParseResult parsed = grav_cmd::CommandParser::parseLine("load_config 'configs/release build.ini'", 7u);
    ASSERT_TRUE(parsed.ok);
    ASSERT_EQ(parsed.requests.size(), 1u);
    ASSERT_EQ(parsed.requests[0].arguments.size(), 1u);
