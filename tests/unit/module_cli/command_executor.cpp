...
TEST(CommandExecutorTest, TST_UNT_MODCLI_009_SetDtMapsToProtocolCommand)
{
    ...
    const grav_cmd::CommandResult result = grav_cmd::CommandExecutor::execute(parseSingle("set_dt 0.25"), context);
...
TEST(CommandExecutorTest, TST_UNT_MODCLI_010_RunStepsAndRunUntilUseDeterministicStepFlow)
{
    ...
    const grav_cmd::CommandResult runSteps = grav_cmd::CommandExecutor::execute(parseSingle("run_steps 3"), context);
    ...
    const grav_cmd::CommandResult runUntil = grav_cmd::CommandExecutor::execute(parseSingle("run_until 1.0"), context);
