...
TEST(CommandBatchRunnerTest, TST_UNT_MODCLI_020_RunScriptFileReportsOpenError)
{
    ...
    const std::string path = (std::filesystem::temp_directory_path() / "grav_missing_script_42861.txt").string();
    std::filesystem::remove(path);

    const grav_cmd::CommandResult result = grav_cmd::CommandBatchRunner::runScriptFile(path, context);
