...
TEST(ServerProtocolCodecParseEdgeTest, TST_UNT_PROT_046_ParseRequestRejectsInvalidEnvelope)
{
    std::string error;
    bltzr_protocol::CommandRequest request;
    EXPECT_FALSE(bltzr_protocol::JsonCodec::parseRequest("[]", request, error));
    EXPECT_EQ(error, "invalid json");
}

TEST(ServerProtocolCodecParseEdgeTest,
     TST_UNT_PROT_047_ParseRequestNormalizesWhitespaceAndCase)
{
    std::string error;
    bltzr_protocol::CommandRequest request;
    EXPECT_TRUE(bltzr_protocol::JsonCodec::parseRequest(
...
TEST(ServerProtocolCodecParseEdgeTest,
     TST_UNT_PROT_048_ParseRequestRejectsWhitespaceOnlyCommand)
{
    std::string error;
    bltzr_protocol::CommandRequest request;
    EXPECT_FALSE(
        bltzr_protocol::JsonCodec::parseRequest(R"({"cmd": "   "})", request, error));
...
{
    std::string error;
    bltzr_protocol::ResponseEnvelope envelope;
    EXPECT_FALSE(bltzr_protocol::JsonCodec::parseResponseEnvelope("[]", envelope, error));
    EXPECT_EQ(error, "invalid json");
    EXPECT_FALSE(bltzr_protocol::JsonCodec::parseResponseEnvelope(
...
TEST(ServerProtocolCodecParseEdgeTest, TST_UNT_PROT_050_ReadStringAndBoolRejectMalformedTokens)
{
    ...
    EXPECT_FALSE(bltzr_protocol::JsonCodec::readString(R"({"key":"unterminated})", "key",
                                                             stringValue));
    EXPECT_FALSE(
        bltzr_protocol::JsonCodec::readBool(R"({"flag":"true"})", "flag", boolValue));
    EXPECT_TRUE(bltzr_protocol::JsonCodec::readBool(R"({"flag": TRUE})", "flag", boolValue));
    EXPECT_TRUE(boolValue);
}

TEST(ServerProtocolCodecParseEdgeTest,
     TST_UNT_PROT_051_ParseRequestRejectsMissingColonSyntax)
{
    std::string error;
    bltzr_protocol::CommandRequest request;
    EXPECT_FALSE(
         ...
         * @return bltzr_protocol::JsonCodec:: value produced by this contract.
         * @note Keep side effects explicit and preserve deterministic behavior where callers depend
         * on it.
         */
        bltzr_protocol::JsonCodec::parseRequest(R"({"cmd" "status"})", request,
                                                             error));
    EXPECT_EQ(error, "missing cmd");
}
...
{
    std::string value;
    EXPECT_TRUE(bltzr_protocol::JsonCodec::readString(
...
{
    bool value = true;
    EXPECT_FALSE(bltzr_protocol::JsonCodec::readBool(R"({"ok": })", "ok", value));
    EXPECT_TRUE(value);
}
     ...
     TST_UNT_PROT_054_ParseResponseEnvelopeKeepsProvidedErrorMessage)
{
    std::string error;
    bltzr_protocol::ResponseEnvelope envelope;
    EXPECT_TRUE(bltzr_protocol::JsonCodec::parseResponseEnvelope(
     ...
     TST_UNT_PROT_055_ParseSnapshotResponseRejectsMissingParticlesArray)
{
    ...
    bltzr_protocol::SnapshotPayload payload{};
    std::string error;
    EXPECT_FALSE(bltzr_protocol::JsonCodec::parseSnapshotResponse(raw, payload, error));
     ...
     TST_UNT_PROT_057_ParseStatusResponseAcceptsErrorEnvelopeWithoutMetrics)
{
    ...
    bltzr_protocol::StatusPayload payload{};
    std::string error;
    ASSERT_TRUE(bltzr_protocol::JsonCodec::parseStatusResponse(raw, payload, error));
     ...
     TST_UNT_PROT_058_ParseSnapshotResponseFallsBackSourceCountWhenMissing)
{
    ...
    bltzr_protocol::SnapshotPayload payload{};
    std::string error;
    ASSERT_TRUE(bltzr_protocol::JsonCodec::parseSnapshotResponse(raw, payload, error));
