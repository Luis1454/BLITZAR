...
TEST(BlitzarCoreApiTest, TST_UNT_CORE_003_ExportsAndReloadsState)
{
    ...
    ASSERT_EQ(blitzar_core_export_state(source, exportPath.string().c_str(), "vtk", 5000u), BLITZAR_CORE_OK);
    ASSERT_TRUE(std::filesystem::exists(exportPath));
    blitzar_core_destroy(source);

    blitzar_core_t *target = createCore(config);
    ASSERT_NE(target, nullptr);
    ASSERT_EQ(blitzar_core_load_state(target, exportPath.string().c_str(), "vtk", 5000u), BLITZAR_CORE_OK);
