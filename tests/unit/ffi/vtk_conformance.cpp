...
static std::vector<blitzar_render_particle_t> readSnapshot(const blitzar_core_t *core)
{
    ...
    std::vector<blitzar_render_particle_t> particles(snapshot.particles, snapshot.particles + snapshot.count);
    blitzar_core_free_snapshot(&snapshot);
    return particles;
}

static void expectParticleNear(const blitzar_render_particle_t &actual, const ExpectedCoreParticle &expected)
{
...
{
    const auto runRoundTrip = [](const char *fixtureName, const char *exportFormat, const char *encodingMarker) {
        ...
        ASSERT_EQ(blitzar_core_export_state(source, exportPath.string().c_str(), exportFormat, 5000u), BLITZAR_CORE_OK);
...
TEST(BlitzarCoreApiTest, TST_UNT_CORE_007_InvalidAndCorruptVtkInputsFallBackDeterministically)
{
    const auto expectGeneratedFallback = [](const std::filesystem::path &path, const char *format) {
        ...
        ASSERT_EQ(blitzar_core_load_state(core, path.string().c_str(), format, 5000u), BLITZAR_CORE_OK);
