...
TEST(ConfigArgsTest, TST_UNT_CONF_037_CliAcceptsOctreeCriterionAndAutoThetaRange)
{
    ...
    const std::vector<std::string> args{
        "app",
        "--solver", "octree_cpu",
        "--octree-opening-criterion", "bounds",
        "--octree-theta-auto-tune", "true",
        "--octree-theta-auto-min", "0.3",
        "--octree-theta-auto-max", "0.9"
    };
...
TEST(ConfigArgsTest, TST_UNT_CONF_038_LoadSupportsOctreeCriterionAndAutoThetaDirective)
{
    ...
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / ("gravity_config_octree_directive_" + std::to_string(stamp) + ".ini");
    {
        std::ofstream out(path, std::ios::trunc);
        ASSERT_TRUE(out.is_open());
        out << "simulation(particle_count=32, dt=0.02, solver=octree_cpu, integrator=euler)\n";
        out << "octree(theta=0.7, softening=0.02, criterion=bounds, theta_auto=true, theta_auto_min=0.3, theta_auto_max=0.9)\n";
    }
...
TEST(ConfigArgsTest, TST_UNT_CONF_039_LoadRejectsInvalidOctreeCriterionAndAutoThetaRange)
{
    const auto stamp = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / ("gravity_config_octree_invalid_" + std::to_string(stamp) + ".ini");
    {
        std::ofstream out(path, std::ios::trunc);
        ASSERT_TRUE(out.is_open());
        out << "simulation(particle_count=32, dt=0.02, solver=octree_cpu, integrator=euler)\n";
        out << "octree(theta=0.7, softening=0.02, criterion=bad_mode, theta_auto=true, theta_auto_min=0.9, theta_auto_max=0.3)\n";
    }
