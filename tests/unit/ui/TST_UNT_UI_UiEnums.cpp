...
#include "src/support/types/GuiEnums.hpp"
...
TEST(UiEnumsTest, SolverConverters)
{
    ...
    EXPECT_EQ(bltzr_qt::to_string(bltzr_qt::Solver::FmmCpu), "fmm_cpu");
    ...
    EXPECT_EQ(bltzr_qt::solver_from_string("fmm_cpu"), bltzr_qt::Solver::FmmCpu);
}
...
TEST(UiEnumsTest, IntegratorConverters)
{
    ...
    EXPECT_EQ(bltzr_qt::to_string(bltzr_qt::Integrator::Leapfrog), "leapfrog");
    EXPECT_EQ(bltzr_qt::integrator_from_string("rk4"), bltzr_qt::Integrator::Rk4);
    EXPECT_EQ(bltzr_qt::integrator_from_string("leapfrog"), bltzr_qt::Integrator::Leapfrog);
}
