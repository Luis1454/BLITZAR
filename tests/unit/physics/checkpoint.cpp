...
static std::filesystem::path makeTempCheckpointPath(const char *stem)
{
    ...
    return std::filesystem::temp_directory_path() / (std::string(stem) + "_" + std::to_string(stamp) + ".chk");
}
...
{
    const std::filesystem::path checkpointPath = makeTempCheckpointPath("grav_checkpoint_roundtrip");
    const std::filesystem::path roundTripPath = makeTempCheckpointPath("grav_checkpoint_roundtrip_reload");
