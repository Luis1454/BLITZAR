...
static std::filesystem::path makeTempExportPath(const char *stem)
{
    ...
    return std::filesystem::temp_directory_path() / (std::string(stem) + "_" + std::to_string(stamp) + ".vtk");
}

static bool waitForExportCompletion(
    SimulationServer &server,
    const std::filesystem::path &path,
    std::uint64_t expectedCompletedCount,
    ...
    bool &outObservedBacklog)
{
    ...
    while (std::chrono::steady_clock::now() <= deadline) {
        ...
        if (std::filesystem::exists(path)
            && stats.exportCompletedCount >= expectedCompletedCount
            && !stats.exportActive
            && stats.exportQueueDepth == 0u) {
            return true;
        }
