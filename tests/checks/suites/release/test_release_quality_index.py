...
def _read_index(archive: Path) -> tuple[dict[str, object], list[str]]:
    with zipfile.ZipFile(archive) as bundle:
        payload = json.loads(bundle.read("release_quality_index.json"))
...
def test_release_quality_index_packages_selected_requirements(tmp_path: Path) -> None:
    ...
    payload, names = _read_index(archive)
...
def test_release_quality_index_defaults_to_all_requirements(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
    archive = ReleaseQualityIndexBuilder().package(
        root=tmp_path,
        dist_dir=tmp_path / "dist/release-quality-index",
    ...
    payload, _ = _read_index(archive)
    assert isinstance(payload["summary"], dict)
    assert payload["summary"]["requirements"] == 2


def test_release_quality_index_rejects_unknown_requirement(tmp_path: Path) -> None:
    _seed_repo(tmp_path)
