...
def _context(root: Path, build_dir: Path, **overrides: object) -> CheckContext:
    payload = {
        "root": root,
        "build_dir": build_dir,
...
def test_clang_tidy_runs_only_changed_compile_entries(monkeypatch, tmp_path: Path) -> None:
    ...
    result = check.run(
        _context(
            tmp_path,
            build_dir,
            clang_tidy_diff_base="origin/main",
            clang_tidy_diff_target="HEAD",
            clang_tidy_header_filter="project",
        )
    )
...
def test_clang_tidy_skips_clean_diffs(monkeypatch, tmp_path: Path) -> None:
    ...
    check = ClangTidyCheck()
    check._runner = _FakeRunner(diff_output="docs/quality/README.md\n")
    monkeypatch.setattr("shutil.which", lambda binary: f"/usr/bin/{binary}")

    result = check.run(_context(tmp_path, build_dir, clang_tidy_diff_base="origin/main"))
...
def test_clang_tidy_reports_git_diff_failures(monkeypatch, tmp_path: Path) -> None:
    ...
    _write_compile_db(build_dir, [source])
    check = ClangTidyCheck()
    check._runner = _FakeRunner(diff_rc=1)
    monkeypatch.setattr("shutil.which", lambda binary: f"/usr/bin/{binary}")
