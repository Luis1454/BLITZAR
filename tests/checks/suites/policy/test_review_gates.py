...
"""
    ...
    def fetch_items(repo: str, number: int, suffix: str, token: str, result):  # noqa: ARG001
        return [{"filename": "runtime/client/runtime/CliRuntime.cpp"}] if suffix == "files" else [{"state": "APPROVED", "user": {"login": "reviewer"}}]
...
def test_traceability_gate_passes_for_critical_pr_with_ids_and_csv(tmp_path: Path) -> None:
    ...
    check = FakeTraceabilityGateCheck([{"filename": "runtime/client/runtime/CliRuntime.cpp"}, {"filename": "docs/quality/traceability.csv"}])
