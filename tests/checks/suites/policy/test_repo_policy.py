...
@pytest.mark.parametrize(("stem", "content", "should_pass"), [
...
def test_repo_policy_enforces_qt_layout_ownership(tmp_path: Path, stem: str, content: str, should_pass: bool) -> None:
    ...
    assert (not errors) if should_pass else any("Qt '*new + reference' ownership pattern is forbidden" in error for error in errors)
...
def test_repo_policy_validates_header_form(tmp_path: Path, name: str, content: str, ok_expected: bool, error_substring: str) -> None:
    _write(tmp_path / "engine" / "include" / name, content)
    ok, errors, _ = _run(tmp_path, tmp_path / "allowlist.txt")
...
def test_repo_policy_validates_workflow_commands(tmp_path: Path, workflow: str, content: str, ok_expected: bool, error_substring: str) -> None:
    _write(tmp_path / ".github" / "workflows" / workflow, content)
    ok, errors, _ = _run(tmp_path, tmp_path / "allowlist.txt")
    assert ok is ok_expected
    assert (not errors) if ok_expected else any(error_substring in error for error in errors)
