...
class TraceabilityGateCommand(BaseCliCommand):
    def build_parser(self) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(description="Validate requirement traceability for critical pull requests.")
        ...
        parser.add_argument("--repo", default=os.getenv("GITHUB_REPOSITORY", ""), help="GitHub owner/repo slug")
        return parser
    ...
    def build_context(self, args: argparse.Namespace) -> CheckContext:
        return CheckContext(
            ...
            options={"repo": args.repo, "token": os.getenv("GITHUB_TOKEN", "")},
        )

    def build_check(self, args: argparse.Namespace) -> BaseCheck:
        del args
        return TraceabilityGateCheck()


def main() -> int:
    return TraceabilityGateCommand().main()
