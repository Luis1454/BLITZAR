...
_Static_assert(offsetof(blitzar_barnes_hut_config_v2, struct_size) == 0,
    "configuration size is first");
...
int main(void)
{
    ...
    BLITZAR_CHECK(blitzar_simulation_set_solver(simulation, BLITZAR_SOLVER_DIRECT) ==
                  BLITZAR_STATUS_OK);

    BLITZAR_CHECK(blitzar_simulation_set_integrator(simulation,
                      BLITZAR_INTEGRATOR_LEAPFROG_KDK) == BLITZAR_STATUS_OK);
    ...
    BLITZAR_CHECK(blitzar_simulation_get_state(simulation, 2, output_x, output_y, output_z,
                      output_velocity_x, output_velocity_y, output_velocity_z, output_mass) ==
                  BLITZAR_STATUS_OK);
    ...
    BLITZAR_CHECK(blitzar_simulation_set_particles_v2(simulation, &input) ==
