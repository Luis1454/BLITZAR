...
int main()
{
    ...
    for (std::size_t index = 0; index < 4; ++index) {
        BLITZAR_CHECK(direct_particles.SetPosition(index, positions[index]) == BLITZAR_STATUS_OK);
    ...
    for (std::size_t index = 0; index < 2; ++index) {
        BLITZAR_CHECK(zero_direct.SetPosition(index, {0.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
        BLITZAR_CHECK(zero_tree.SetPosition(index, {0.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
    ...
    BLITZAR_CHECK(tight_particles.SetPosition(0, {-1.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
    BLITZAR_CHECK(tight_particles.SetPosition(1, {1.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
    ...
    BLITZAR_CHECK(tight_tree.Build(tight_particles.State()) == BLITZAR_STATUS_OK);
    ...
    BLITZAR_CHECK(tree_singular.SetPosition(0, {0.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
    BLITZAR_CHECK(tree_singular.SetPosition(1, {1.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);
    BLITZAR_CHECK(tree_singular.SetPosition(2, {1.0, 0.0, 0.0}) == BLITZAR_STATUS_OK);

    for (std::size_t index = 0; index < 3; ++index) {
        BLITZAR_CHECK(tree_singular.SetMass(index, 1.0) == BLITZAR_STATUS_OK);
    }

    const blitzar_core::ForceView tree_singular_view = tree_singular_force.View();
    ...
    blitzar_barnes_hut::BarnesHutSolver clustered_solver(gravity, clustered_settings);
    ...
    BLITZAR_CHECK(clustered_tree.SetPosition(0, {-4.95, -5.0, -5.0}) == BLITZAR_STATUS_OK);
