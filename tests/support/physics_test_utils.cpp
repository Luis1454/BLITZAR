...
static bool waitForSnapshot(
    const std::function<bool(std::vector<RenderParticle> &)> &snapshotReader,
...
bool waitForConsumedSnapshot(SimulationServer &server, std::vector<RenderParticle> &outSnapshot, int timeoutMs)
{
    return waitForSnapshot(
        [&server](std::vector<RenderParticle> &snapshot) { return server.tryConsumeSnapshot(snapshot); },
        outSnapshot,
        timeoutMs);
}
...
{
    return waitForSnapshot(
        [&server](std::vector<RenderParticle> &snapshot) { return server.copyLatestSnapshot(snapshot); },
        outSnapshot,
        timeoutMs);
}
...
bool haveExactReplayMatch(const ScenarioResult &lhs, const ScenarioResult &rhs, std::string &error)
{
    ...
    for (std::size_t i = 0; i < lhs.final.size(); ++i) {
        if (lhs.final[i].x != rhs.final[i].x
            || lhs.final[i].y != rhs.final[i].y
