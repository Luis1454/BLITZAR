...
Section "BLITZAR" SecMain
    ...
    CreateShortcut "$SMPROGRAMS\BLITZAR\BLITZAR GUI.lnk" "$INSTDIR\blitzar-gui.exe" "--config $\"$INSTDIR\simulation.ini$\"" "$INSTDIR\blitzar-gui.exe"
    CreateShortcut "$SMPROGRAMS\BLITZAR\Uninstall BLITZAR.lnk" "$INSTDIR\Uninstall BLITZAR.exe"
    CreateShortcut "$DESKTOP\BLITZAR GUI.lnk" "$INSTDIR\blitzar-gui.exe" "--config $\"$INSTDIR\simulation.ini$\"" "$INSTDIR\blitzar-gui.exe"
