...
Section "BLITZAR" SecMain
    ...
    CreateShortcut "$SMPROGRAMS\BLITZAR\BLITZAR GUI.lnk" "$INSTDIR\blitzar-client.exe" "--config $\"$INSTDIR\simulation.ini$\" --module qt --wait-for-module" "$INSTDIR\blitzar-client.exe"
    CreateShortcut "$SMPROGRAMS\BLITZAR\Uninstall BLITZAR.lnk" "$INSTDIR\Uninstall BLITZAR.exe"
    CreateShortcut "$DESKTOP\BLITZAR GUI.lnk" "$INSTDIR\blitzar-client.exe" "--config $\"$INSTDIR\simulation.ini$\" --module qt --wait-for-module" "$INSTDIR\blitzar-client.exe"
