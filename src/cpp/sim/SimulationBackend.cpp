...
SimulationBackend::SimulationBackend(const std::string &configPath)
    : SimulationBackend(2u, 0.01f)
{
...
void SimulationBackend::requestRecover()
{
    requestReset();
}
...
bool SimulationBackend::copyLatestSnapshot(std::vector<RenderParticle> &outSnapshot, std::size_t maxPoints) const
{
    std::lock_guard<std::mutex> lock(_snapshotMutex);
    if (_publishedSnapshot.empty()) {
        outSnapshot.clear();
...
SimulationConfig SimulationBackend::getRuntimeConfig() const
{
...
void SimulationBackend::rebuildSystem()
{
    ...
    if (solverModeFromCanonicalName(solver) == ParticleSystem::SolverMode::PairwiseCuda
        ...
        if (isAutoSolverFallbackEnabled()) {
            ...
            std::cout << "[backend] pairwise_cuda with " << targetParticleCount
                      << " particles is not realtime; auto-switching to octree_gpu\n";
        } else {
            std::cout << "[backend] warning: pairwise_cuda with " << targetParticleCount
        ...
        if (_resetRequested.exchange(false, std::memory_order_relaxed)) {
            if (_cudaContextDirty.exchange(false, std::memory_order_relaxed)) {
                ...
                if (resetStatus != cudaSuccess) {
                    std::cerr << "[backend] cudaDeviceReset failed: "
                              << cudaGetErrorString(resetStatus) << "\n";
                } else {
                    std::cout << "[backend] CUDA context reset after previous failure\n";
                }
            }
            rebuildSystem();
        }
