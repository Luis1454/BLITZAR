...
const std::string_view kBackendDefaultName = sim::platform::backendDefaultExecutableName();

bool parsePortValue(std::string_view raw, std::uint16_t &outPort)
{
    unsigned int parsed = 0u;
    if (!sim::text::parseNumber(raw, parsed) || parsed == 0u || parsed > 65535u) {
        return false;
    }
...
void FrontendBackendBridge::stop()
{
    ...
    if (_remoteMode) {
        if (_remoteBackendLaunched && _remoteClient.isConnected()) {
            (void)_remoteClient.sendCommand(std::string(sim::protocol::cmd::Shutdown));
        }
        if (_remoteConnected) {
...
void FrontendBackendBridge::setPaused(bool paused)
{
    ...
    dispatchBackendCall(
        ...
        [this, paused]() {
            sendOrQueueRemote(std::string(paused ? sim::protocol::cmd::Pause : sim::protocol::cmd::Resume));
        });
}
...
void FrontendBackendBridge::togglePaused()
{
    ...
    dispatchBackendCall(
        ...
        [this]() { sendOrQueueRemote(std::string(sim::protocol::cmd::Toggle)); });
}
...
void FrontendBackendBridge::stepOnce()
{
    ...
    dispatchBackendCall(
        ...
        [this]() { sendOrQueueRemote(std::string(sim::protocol::cmd::Step), "\"count\":1"); });
}
...
void FrontendBackendBridge::setParticleCount(std::uint32_t particleCount)
{
    ...
    dispatchBackendCall(
        ...
        [this, particleCount]() {
            ...
            sendOrQueueRemote(std::string(sim::protocol::cmd::SetParticleCount), "\"value\":" + std::to_string(clamped));
        });
}
...
void FrontendBackendBridge::setDt(float dt)
{
    ...
    dispatchBackendCall(
        ...
        [this, dt]() {
            ...
            sendOrQueueRemote(std::string(sim::protocol::cmd::SetDt), "\"value\":" + std::to_string(clamped));
        });
}
...
void FrontendBackendBridge::setSolverMode(const std::string &mode)
{
    ...
    dispatchBackendCall(
        ...
        [this, &mode]() {
            sendOrQueueRemote(std::string(sim::protocol::cmd::SetSolver), "\"value\":\"" + jsonEscape(mode) + "\"");
        });
}
...
void FrontendBackendBridge::setIntegratorMode(const std::string &mode)
{
    ...
    dispatchBackendCall(
        ...
        [this, &mode]() {
            sendOrQueueRemote(std::string(sim::protocol::cmd::SetIntegrator), "\"value\":\"" + jsonEscape(mode) + "\"");
        });
}
...
void FrontendBackendBridge::setOctreeParameters(float theta, float softening)
{
    ...
    dispatchBackendCall(
        ...
        [this, theta, softening]() {
            ...
            sendOrQueueRemote(
                std::string(sim::protocol::cmd::SetOctree),
...
void FrontendBackendBridge::setSphEnabled(bool enabled)
{
    ...
    dispatchBackendCall(
        ...
        [this, enabled]() {
            sendOrQueueRemote(std::string(sim::protocol::cmd::SetSph), std::string("\"value\":") + (enabled ? "true" : "false"));
        });
}
...
void FrontendBackendBridge::setSphParameters(float smoothingLength, float restDensity, float gasConstant, float viscosity)
{
    ...
    dispatchBackendCall(
        ...
        [this, smoothingLength, restDensity, gasConstant, viscosity]() {
            ...
            sendOrQueueRemote(
                std::string(sim::protocol::cmd::SetSphParams),
...
void FrontendBackendBridge::setEnergyMeasurementConfig(std::uint32_t everySteps, std::uint32_t sampleLimit)
{
    ...
    dispatchBackendCall(
        ...
        [this, everySteps, sampleLimit]() {
            ...
            sendOrQueueRemote(
                std::string(sim::protocol::cmd::SetEnergyMeasure),
...
void FrontendBackendBridge::setInitialStateFile(const std::string &path, const std::string &format)
{
    ...
    dispatchBackendCall(
        ...
        [this, &path, &format]() {
            if (!path.empty()) {
                sendOrQueueRemote(
                    std::string(sim::protocol::cmd::Load),
...
void FrontendBackendBridge::requestExportSnapshot(const std::string &outputPath, const std::string &format)
{
    ...
    dispatchBackendCall(
        ...
        [this, &outputPath, &format]() {
            ...
            sendOrQueueRemote(std::string(sim::protocol::cmd::Export), fields);
        });
}
...
void FrontendBackendBridge::requestShutdown()
{
    ...
    dispatchBackendCall(
        ...
        [this]() { sendOrQueueRemote(std::string(sim::protocol::cmd::Shutdown)); });
}
...
bool FrontendBackendBridge::tryConsumeSnapshot(std::vector<RenderParticle> &outSnapshot)
{
    ...
    const BackendClientResponse response = _remoteClient.getSnapshot(
        remoteSnapshot,
        sim::protocol::clampSnapshotPoints(_remoteSnapshotCap));
    if (!response.ok) {
...
{
    std::lock_guard<std::recursive_mutex> lock(_mutex);
    _remoteSnapshotCap = sim::protocol::clampSnapshotPoints(maxPoints);
}
