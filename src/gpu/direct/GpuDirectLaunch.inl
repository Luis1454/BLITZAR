...
[[nodiscard]] blitzar_status ValidateLaunch(const DirectLaunchRequest& request) noexcept
{
    ...
    const blitzar_solvers::ForceRange& range = request.range;
...
blitzar_status LaunchDirect(const DirectLaunchRequest& request) noexcept
{
    ...
    const bool source_alias_target = range.source_begin == 0 &&
                                     range.source_end == request.target_count &&
    ...
    const DirectDeviceRequest kernel_request{reinterpret_cast<const double*>(addresses.position_x),
        ...
        reinterpret_cast<const double*>(
            source_alias_target ? addresses.position_x : addresses.source_position_x),
        reinterpret_cast<const double*>(
            source_alias_target ? addresses.position_y : addresses.source_position_y),
        reinterpret_cast<const double*>(
            source_alias_target ? addresses.position_z : addresses.source_position_z),
        reinterpret_cast<const double*>(
            source_alias_target ? addresses.mass : addresses.source_mass),
        request.target_count, range.source_begin, range.source_end, request.source_global_begin,
        physics.gravitational_constant, physics.softening, range.accumulate,
