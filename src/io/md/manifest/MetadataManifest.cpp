...
blitzar_status MetadataRunInfo::Validate() const noexcept
{
    ...
    const bool expected_bitwise_reproducible =
        execution.mode == blitzar_core::ExecutionMode::Strict &&
        simulation.solver == BLITZAR_SOLVER_DIRECT;

    if (execution.bitwise_reproducible != expected_bitwise_reproducible) {
        return BLITZAR_STATUS_INVALID_ARGUMENT;
    }
