    ...
    std::size_t max_depth) noexcept
{
    ...
    const std::size_t depth_capacity =
        max_depth > (maximum - 1) / (ChildrenPerCell - 1)
            ? maximum
...
ThreadWorkspace::ThreadWorkspace(
    ...
    : max_cells_(max_cells),
      ...
      thread_count_(DetectThreadCount()),
      storage_{}
{
    if (stack_capacity_ != 0 &&
        thread_count_ >
            std::numeric_limits<std::size_t>::max() / stack_capacity_) {
...
std::span<std::size_t> ThreadWorkspace::Stack(
