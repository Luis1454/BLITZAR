...
bool BhSolver::IsValidState(blitzar_core::ParticleStateView particles) noexcept
{
...
blitzar_status BhSolver::AccumulateSource(const AccumulationRequest& request, std::size_t source,
    blitzar_core::Vector3 target_position) const noexcept
{
    ...
    const blitzar_core::Scalar distance_squared = displacement.x * displacement.x +
...
blitzar_status BhSolver::AccumulateLeaf(const AccumulationRequest& request,
...
blitzar_status BhSolver::AccumulateMultipole(const AccumulationRequest& request,
    ...
    bool& consumed) const noexcept
{
    ...
    const bool contains = periodic_enabled ? request.periodic->Contains(
                                                 cell.center, cell.half_extent, target_position)
                                           : Contains(cell, target_position);

    if (contains || distance <= 0.0 || cell_width / distance >= settings_.opening_angle) {
        return BLITZAR_STATUS_OK;
    }
...
blitzar_status BhSolver::PushChildren(const blitzar_trees::Octree::Cell& cell,
...
blitzar_status BhSolver::Accumulate(const AccumulationRequest& request) noexcept
{
...
blitzar_status BhSolver::ComputeTree(const TreeComputeRequest& request) noexcept
{
