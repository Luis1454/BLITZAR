...
blitzar_status MpiDomainDecomposition::ValidateInput(
    blitzar_core::ParticleStateView state, const MpiContext& context) const noexcept
{
    ...
    const blitzar_status synchronization_status =
        context.SynchronizeStatus(valid ? BLITZAR_STATUS_OK : BLITZAR_STATUS_INVALID_ARGUMENT,
            "MpiDomainDecomposition", "initialize-preflight", global_status);
...
blitzar_status MpiDomainDecomposition::InitializeBounds(
    blitzar_core::ParticleStateView state, const MpiContext& context) noexcept
{
    MpiDomainBounds bounds = MpiDomainBounds::From(state);
...
void MpiDomainDecomposition::UpdateLocalBounds(blitzar_core::ParticleStateView state) noexcept
{
    local_bounds_ = {};
