...
bool MpiDomainBounds::IsValid() const noexcept
{
    return IsFinite(minimum) && IsFinite(maximum) && minimum.x <= maximum.x &&
           minimum.y <= maximum.y && minimum.z <= maximum.z;
}

bool MpiDomainBounds::Contains(blitzar_core::Vector3 position) const noexcept
{
...
bool MpiDomainBounds::Include(blitzar_core::Vector3 position) noexcept
{
...
MpiDomainBounds MpiDomainBounds::From(blitzar_core::ParticleStateView state) noexcept
{
    MpiDomainBounds bounds{{PositiveInfinity, PositiveInfinity, PositiveInfinity},
    ...
    return bounds.IsValid() ? bounds : MpiDomainBounds{};
}
