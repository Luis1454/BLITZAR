...
[[nodiscard]] blitzar_status SetLocalCounts(
    SimPacketStoreRequest& request, std::size_t count, CountSnapshot previous) noexcept
{
...
void WriteLocalPackets(SimPacketStoreRequest& request) noexcept
{
...
blitzar_status StoreGhosts(blitzar_parallel::PacketBuffer& ghosts,
    blitzar_particles::ParticleSourceBuffer& source) noexcept
{
    if (!source.IsValid()) {
...
blitzar_status StoreLocalPackets(SimPacketStoreRequest& request) noexcept
{
