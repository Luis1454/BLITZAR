    ...
    std::size_t rollback_particle_count, StepTransaction& transaction) noexcept
{
    ...
    const blitzar_status state_status = blitzar_parallel::SynchronizeStatus(resources_.Mpi(),
    ...
    return blitzar_parallel::SynchronizeStatus(resources_.Mpi(), prepare_status, "step-prepare");
}
