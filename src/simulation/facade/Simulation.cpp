...
Simulation::Simulation(std::size_t particle_count)
    : particle_count_(particle_count), resources_(particle_count),
      particle_storage_(particle_count), source_(particle_count), gravity_{},
      barnes_hut_{
      ...
      last_status_(resources_.Mpi().Status()), last_backend_(BLITZAR_BACKEND_CPU),
...
{
    const blitzar_status capacity_status = resources_.Exchange().CapacityStatus();
...
void Simulation::SetFaultForTesting(blitzar_hip::Fault fault) noexcept
{
    resources_.Accelerator().SetFaultForTesting(fault);
}
...
{
    return resources_.Mpi().Rank();
}

int Simulation::MpiSize() const noexcept
{
    return resources_.Mpi().Size();
}
