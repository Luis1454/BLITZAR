...
{
    const bool root = resources_.Mpi().Rank() == 0;
...
blitzar_status Simulation::DistributeParticles(ParticleInputStage& stage,
    blitzar_parallel::DomainDecomposition& domain,
    blitzar_parallel::PacketBuffer& distributed) noexcept
{
    const bool root = resources_.Mpi().Rank() == 0;
    const std::size_t local_capacity = LocalCapacity(particle_count_, resources_.Mpi().Size());
...
{
    if (!resources_.Mpi().IsUsable()) {
        return Remember(resources_.Mpi().Status());
    }

    blitzar_status status = blitzar_parallel::SynchronizeStatus(resources_.Mpi(),
    ...
    status = blitzar_parallel::SynchronizeStatus(resources_.Mpi(), status, "set-particles-stage");
    ...
    status = candidate_domain.Initialize(stage.State(), resources_.Mpi());
    status = blitzar_parallel::SynchronizeStatus(resources_.Mpi(), status, "set-particles-domain");
    ...
    status =
        blitzar_parallel::SynchronizeStatus(resources_.Mpi(), status, "set-particles-distribution");
    ...
    status = blitzar_parallel::SynchronizeStatus(resources_.Mpi(), status, "set-particles-store");

    if (status != BLITZAR_STATUS_OK) {
        return Remember(status);
    }

    resources_.Domain() = std::move(candidate_domain);
