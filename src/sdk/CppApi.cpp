...
Context::Context() noexcept
    : impl_(nullptr), status_(Status::InvalidArgument)
{
    try {
...
{
    return Update(blitzar_simulation_set_solver(
        impl_ == nullptr ? nullptr : impl_->handle.get(),
...
{
    return Update(blitzar_simulation_set_integrator(
        impl_ == nullptr ? nullptr : impl_->handle.get(),
        static_cast<blitzar_integrator_kind>(integrator)));
}

Status Simulation::set_gravity(
...
Status Simulation::set_barnes_hut(
...
{
    return Update(blitzar_simulation_set_barnes_hut(
        impl_ == nullptr ? nullptr : impl_->handle.get(),
        opening_angle,
        max_particles,
        max_cells,
...
{
    return Update(blitzar_simulation_set_seed(
        impl_ == nullptr ? nullptr : impl_->handle.get(), seed));
}

Status Simulation::set_particles(
...
{
    if (position_x.size() != position_y.size() ||
        position_x.size() != position_z.size() ||
        position_x.size() != velocity_x.size() ||
        position_x.size() != velocity_y.size() ||
        position_x.size() != velocity_z.size() ||
        position_x.size() != mass.size() || !FitsCount(position_x.size())) {
        return Update(BLITZAR_STATUS_INVALID_ARGUMENT);
    }
    return Update(blitzar_simulation_set_particles(
        impl_ == nullptr ? nullptr : impl_->handle.get(),
        static_cast<std::int64_t>(position_x.size()),
        position_x.data(),
        position_y.data(),
        position_z.data(),
        velocity_x.data(),
    ...
    std::span<double> mass) noexcept
{
    ...
    return Update(blitzar_simulation_get_state(
        impl_ == nullptr ? nullptr : impl_->handle.get(),
        static_cast<std::int64_t>(position_x.size()),
        position_x.data(),
        position_y.data(),
...
{
    return Update(blitzar_simulation_step(
        impl_ == nullptr ? nullptr : impl_->handle.get()));
}
