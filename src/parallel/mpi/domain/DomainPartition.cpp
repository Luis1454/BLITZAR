#include "parallel/mpi/domain/DomainPartition.hpp"
...
blitzar_status DomainPartition::AllocateBuffers(
...
void DomainPartition::BuildRootKeys(
    blitzar_core::ParticleStateView state, DomainBounds global_bounds) noexcept
{
    for (std::size_t index = 0; index < state.SourceCount(); ++index) {
        keys_[index] =
            blitzar_tree_ordering::MortonKey({state.x[index], state.y[index], state.z[index]},
...
void DomainPartition::BuildRootSplits() noexcept
{
...
blitzar_status DomainPartition::BroadcastSplits(const MpiContext& context) noexcept
{
...
blitzar_status DomainPartition::Initialize(blitzar_core::ParticleStateView state,
...
int DomainPartition::Owner(blitzar_core::Vector3 position, std::uint64_t particle_id,
    DomainBounds global_bounds) const noexcept
{
    ...
    const SplitKey candidate{
        blitzar_tree_ordering::MortonKey(position, global_bounds.minimum, global_bounds.maximum),
        particle_id};
