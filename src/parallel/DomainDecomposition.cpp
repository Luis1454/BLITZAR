...
{
    return IsFinitePosition(minimum) && IsFinitePosition(maximum) &&
           minimum.x <= maximum.x && minimum.y <= maximum.y &&
           minimum.z <= maximum.z;
}

DomainBounds DomainDecomposition::BoundsOf(
...
bool DomainDecomposition::Extend(
    ...
    const MpiContext& context) noexcept
{
    ...
    for (std::size_t index = 0; index < global_state.SourceCount(); ++index) {
        keys[index] = blitzar_trees::MortonKey(
            ...
            global_bounds_.minimum,
            global_bounds_.maximum);
        order[index] = index;
    }
    std::sort(order.begin(), order.end(), [&keys](const auto left, const auto right) {
        return keys[left] < keys[right] ||
               (keys[left] == keys[right] && left < right);
    });
    ...
    blitzar_core::Vector3 position, std::uint64_t particle_id) const noexcept
{
    ...
    const std::uint64_t key = blitzar_trees::MortonKey(
        position, global_bounds_.minimum, global_bounds_.maximum);
