...
[[nodiscard]] bool ValidAllToAllCounts(
    std::span<const int> send_counts, std::span<int> receive_counts, std::size_t peer_count) noexcept
{
...
{
    const bool layout_valid = ValidAllToAllCounts(
        send_counts, receive_counts, static_cast<std::size_t>(session_.Size()));
...
{
#if defined(BLITZAR_HAS_MPI)
    const blitzar_status status = MpiPacketProtocol::SynchronizePreparation(
        collectives_, layout_valid ? BLITZAR_STATUS_OK : BLITZAR_STATUS_INVALID_ARGUMENT,
...
{
    const bool layout_valid = ValidAllGatherCount(
        local_count, counts, static_cast<std::size_t>(session_.Size()));
...
blitzar_status MpiPacketTransport::ExecuteAllGatherCounts(bool layout_valid,
    int local_count, std::span<int> counts) const noexcept
{
#if defined(BLITZAR_HAS_MPI)
    const blitzar_status status = MpiPacketProtocol::SynchronizePreparation(
