...
from python_tools.core.io import REQ_ID_RE, GitTrackedService
...
class QualityBaselineCheck(BaseCheck):
    ...
    def _execute(self, context: CheckContext, result: CheckResult) -> None:
        ...
        requirements = self._load_requirement_artifacts(manifest, result)
        self._check_requirement_artifacts(context.root, requirements, result)

        # Deliberately does not validate "requirement -> tests" coverage here.
        # That responsibility lives in the test catalog gate.
    ...
    def _check_crosswalk(self, root: Path, manifest: dict[str, JsonValue], result: CheckResult) -> None:
        ...
        for row in rows:
            ...
            if not repo_artifact_ref:
                ...
                continue
            artifact_path, artifact_error = resolve_evidence_ref(root, repo_artifact_ref)
            if artifact_error is not None:
            ...
            if artifact_path is None:
                ...
                continue
            if not (root / artifact_path).exists():
                result.add_error(f"{control_id or '<missing control>'}: artifact does not exist: {repo_artifact_ref} -> {artifact_path}")
                continue
            if artifact_path == "AGENTS.md" and not self._git.is_tracked(root, artifact_path):
