/*
 * @file modules/qt/src/window/workspace/GuiPersistence.cpp
 * @author Luis1454
 * @project BLITZAR
...
namespace bltzr_qt {
void Window::saveWorkspacePreset()
{
    bool accepted = false;
    ...
    _widgets.telemetry.statusLabel->setText(saved ? QString("workspace saved: %1").arg(rawName.trimmed())
...
void Window::loadWorkspacePreset()
{
    const std::vector<std::string> presets = _workspaceLayouts.listPresets();
    if (presets.empty()) {
        _widgets.telemetry.statusLabel->setText("no saved workspace preset");
    ...
    if (!_workspaceLayouts.loadPreset(selected.toStdString(), state, geometry)) {
        _widgets.telemetry.statusLabel->setText("failed to load workspace");
    ...
    _widgets.telemetry.statusLabel->setText((geometryRestored && stateRestored)
...
void Window::deleteWorkspacePreset()
{
    const std::vector<std::string> presets = _workspaceLayouts.listPresets();
    if (presets.empty()) {
        _widgets.telemetry.statusLabel->setText("no workspace preset to delete");
    ...
    _widgets.telemetry.statusLabel->setText(deleted ? QString("workspace deleted: %1").arg(selected)
...
void Window::restoreDefaultWorkspace()
{
