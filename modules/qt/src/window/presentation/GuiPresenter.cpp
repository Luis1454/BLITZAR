/*
 * @file modules/qt/src/window/presentation/GuiPresenter.cpp
 * @author Luis1454
 * @project BLITZAR
...
static std::string latencyLabel(const PresentationInput& input)
    {
...
static float simulatedSecondsPerSecond(const PresentationInput& input)
    {
...
static std::string backendStateLabel(const PresentationInput& input, bool staleStats,
                                         bool staleSnapshot)
    {
...
static std::string progressLabel(const PresentationInput& input)
    {
...
static std::string etaLabel(const PresentationInput& input)
    {
        if (input.simulationHorizonSeconds <= 0.0f ||
...
static std::string exportStateLabel(const PresentationInput& input)
    {
...
{
    const bool staleStats = isStale(input.statsAgeMs);
    const bool staleSnapshot = isStale(input.snapshotAgeMs);
    ...
    const std::string latency = latencyLabel(input);
    const std::string backendState =
        backendStateLabel(input, staleStats, staleSnapshot);
    const std::string viewportState =
        viewportStateLabel(input.snapshotAgeMs);
    const float simRate = simulatedSecondsPerSecond(input);
    const std::string progress = progressLabel(input);
    const std::string eta = etaLabel(input);
    const std::string exportState = exportStateLabel(input);
    Presentation output;
    ...
    runtime << "dt: " << input.stats.dt << " s"
            ...
            << "\nSim rate: " << fixedLabel(simRate, 2) << " sim s/s"
    ...
    if (!input.stats.gpuTelemetryEnabled) {
        ...
        gpu << "State: active"
            << "\nSampling: every 8 steps"
            << "\nKernel: " << fixedLabel(input.stats.gpuKernelMs, 3)
            << " ms"
            << "\nCopy: " << fixedLabel(input.stats.gpuCopyMs, 3) << " ms"
            << "\nVRAM: " << bytesLabel(input.stats.gpuVramUsedBytes)
            << " / " << bytesLabel(input.stats.gpuVramTotalBytes);
    }
    ...
    trace << "[qt] step=" << input.stats.steps << " link=" << input.linkLabel
          ...
          << " sim_rate=" << fixedLabel(simRate, 2) << " progress=\""
          << progress << "\""
          << " eta=\"" << eta << "\""
          ...
          << " stats_age_ms=" << ageLabel(input.statsAgeMs)
          << " snap_age_ms=" << ageLabel(input.snapshotAgeMs)
          ...
          << " gpu_kernel_ms=" << fixedLabel(input.stats.gpuKernelMs, 3)
          << " gpu_copy_ms=" << fixedLabel(input.stats.gpuCopyMs, 3)
          << " gpu_vram_used=\""
          << bytesLabel(input.stats.gpuVramUsedBytes) << "\""
          << " gpu_vram_total=\""
          << bytesLabel(input.stats.gpuVramTotalBytes) << "\""
