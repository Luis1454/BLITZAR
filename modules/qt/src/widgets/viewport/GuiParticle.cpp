/*
 * @file modules/qt/src/widgets/viewport/GuiParticle.cpp
 * @author Luis1454
 * @project BLITZAR
...
#include "src/widgets/overlays/GuiPainter.hpp"
#include "src/widgets/viewport/GuiColor.hpp"
#include <QMouseEvent>
#include <QPaintEvent>
#include <QPainter>
#include <QPen>
...
void Particle::setSnapshot(const std::vector<RenderParticle>& snapshot)
{
    _snapshot = std::cref(snapshot);
    update();
}

void Particle::setMode(grav::ViewMode mode)
{
    _mode = mode;
    update();
}

void Particle::setZoom(float zoom)
{
    _zoom = std::max(kViewportMinZoom, zoom);
    update();
}

void Particle::setLuminosity(int luminosity)
{
    _luminosity = std::clamp(luminosity, kLuminosityMin, kLuminosityMax);
    update();
}

void Particle::setCameraAngles(float yaw, float pitch, float roll)
{
    _camera.yaw = yaw;
    _camera.pitch = pitch;
    _camera.roll = roll;
...
void Particle::setRenderSettings(bool culling, bool lod, float nearDist, float farDist)
{
...
void Particle::setOctreeOverlay(const std::vector<OctreeNode>& overlay, bool enabled,
                                    int opacity)
{
...
void Particle::mousePressEvent(MouseEventHandle event)
{
...
void Particle::mouseMoveEvent(MouseEventHandle event)
{
...
void Particle::mouseReleaseEvent(MouseEventHandle event)
{
    if (_dragAxis != grav::GimbalAxis::None) {
        _dragAxis = grav::GimbalAxis::None;
        event->accept();
...
void Particle::paintEvent(PaintEventHandle event)
{
    (void)event;
    if (!_snapshot) {
        return;
    ...
    if (_octreeOverlayEnabled && _octreeOverlay.has_value()) {
        paintOctreeOverlay(painter, rect(), _mode, _camera, _octreeOverlay->get(), _zoom,
