/*
 * @file modules/qt/src/widgets/overlays/GuiOctree.cpp
 * @author Luis1454
 * @project BLITZAR
...
static void appendNodes(const std::vector<RenderParticle>& particles,
                        const std::vector<std::size_t>& indices, const OctreeNode& node,
                        int maxDepth, std::vector<OctreeNode>& nodes)
{
...
std::vector<OctreeNode> Octree::build(const std::vector<RenderParticle>& particles,
                                                    int maxDepth)
{
    ...
    appendNodes(
        particles, indices,
        OctreeNode{bounds.centerX, bounds.centerY, bounds.centerZ, bounds.halfSize, 0},
        maxDepth, nodes);
    return nodes;
}
} // namespace bltzr_qt
