/*
 * @file modules/qt/src/widgets/overlays/GuiPainter.cpp
 * @author Luis1454
 * @project BLITZAR
...
{
    const int clampedOpacity = std::clamp(opacity, kLuminosityMin, kLuminosityMax);
    const int red = std::clamp(72 + (depth * 28), kLuminosityMin, kLuminosityMax);
    const int green = std::clamp(120 + (depth * 22), kLuminosityMin, kLuminosityMax);
    const int blue = std::clamp(kLuminosityMax - (depth * 16), 80, kLuminosityMax);
...
static void drawOrthographicNode(QPainter& painter, const QRect& viewport, grav::ViewMode mode,
                                 const OctreeNode& node, float zoom)
{
