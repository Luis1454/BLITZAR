...
#include "src/support/theme/GuiTheme.hpp"
...
struct QtInProcState {
    ...
    bltzr_client::TransportArgs transport{
        kDefaultLoopbackHost,
        kDefaultServerPort,
        true, // autostart on by default for GUI launches
        {},
        {},
        bltzr_client::kRemoteCommandTimeoutMsDefault,
        bltzr_client::kRemoteStatusTimeoutMsDefault,
        bltzr_client::kRemoteSnapshotTimeoutMsDefault};
    std::thread uiThread;
...
static std::string siblingServerExecutable(const std::string& modulePath)
{
    if (modulePath.empty()) {
        return {};
    }
    const QFileInfo moduleInfo(QString::fromStdString(modulePath));
    const QDir moduleDir = moduleInfo.absoluteDir();
    const std::string serverExecutable =
        bltzr_platform::executableName(bltzr_platform::serverDefaultExecutableName());
    const QString serverName = QString::fromStdString(serverExecutable);
    const QString serverCandidate = moduleDir.filePath(serverName);
    if (QFileInfo::exists(serverCandidate)) {
        return serverCandidate.toStdString();
    }
    return {};
}
...
static void qtThreadMain(QtInProcState* state)
{
    try {
        ...
        if (state->transport.serverExecutable.empty()) {
            state->transport.serverExecutable = siblingServerExecutable(state->modulePath);
        }
        if (state->transport.serverExecutable.empty()) {
            const QString appDir = QCoreApplication::applicationDirPath();
            if (!appDir.isEmpty()) {
                const std::string serverExecutable =
                    bltzr_platform::executableName(bltzr_platform::serverDefaultExecutableName());
                const QString serverCandidate =
                    QDir(appDir).filePath(QString::fromStdString(serverExecutable));
                if (QFileInfo::exists(serverCandidate)) {
        ...
        state->transport.remoteCommandTimeoutMs =
            bltzr_client::clampRemoteTimeoutMs(config.clientRemoteCommandTimeoutMs);
        state->transport.remoteStatusTimeoutMs =
            bltzr_client::clampRemoteTimeoutMs(config.clientRemoteStatusTimeoutMs);
        state->transport.remoteSnapshotTimeoutMs =
            bltzr_client::clampRemoteTimeoutMs(config.clientRemoteSnapshotTimeoutMs);
        auto runtime =
            std::make_unique<bltzr_client::Runtime>(state->configPath, state->transport);
        bltzr_qt::Window window(config, state->configPath, std::move(runtime));
        window.show();
...
static bool startQtUi(QtInProcState& state, const bltzr_client::ErrorBufferView& errorBuffer)
{
    ...
    for (int i = 0; i < kQtStartupPollAttempts; ++i) {
        if (state.startupDone.load()) {
            break;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(kQtStartupPollIntervalMs));
    }
    if (!state.startupDone.load()) {
...
public:
                       ...
                       const bltzr_client::ErrorBufferView& errorBuffer)
    {
        try {
            ...
            if (context != nullptr && context->modulePath != nullptr) {
                state->modulePath = context->modulePath;
            }
     ...
     * it.
     */
    static QtInProcState* requireState(bltzr_module::OpaqueState moduleState,
                                       const bltzr_client::ErrorBufferView& errorBuffer)
    {
           ...
           std::size_t errorBufferSize) -> bool {
            ...
            try {
                ...
                if (cmd == "set-endpoint") {
                    ...
                    if (!bltzr_text::parseNumber(tokens[2], parsedPort) ||
                        parsedPort < kNetworkPortMin || parsedPort > kNetworkPortMax) {
