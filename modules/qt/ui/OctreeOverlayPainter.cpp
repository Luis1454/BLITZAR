...
static void drawOrthographicNode(
    QPainter &painter,
    const QRect &viewport,
    grav::ViewMode mode,
    const OctreeOverlayNode &node,
    float zoom)
{
    ...
    const QRectF box(
        centerX + (projectedX * zoom) - halfSizePx,
        centerY - (projectedY * zoom) - halfSizePx,
        halfSizePx * 2.0f,
        halfSizePx * 2.0f);
    painter.drawRect(box);
}

static bool projectCorner(
    const QRect &viewport,
    grav::ViewMode mode,
    const grav::CameraState &camera,
    float zoom,
    const RenderParticle &corner,
    QPointF &projected)
{
...
void OctreeOverlayPainter::paint(
    QPainter &painter,
    const QRect &viewport,
    grav::ViewMode mode,
    const grav::CameraState &camera,
    const std::vector<OctreeOverlayNode> &nodes,
    float zoom,
    int opacity)
{
    ...
    static constexpr std::array<std::array<int, 2>, 12> kCubeEdges = {{
        {{0, 1}}, {{1, 2}}, {{2, 3}}, {{3, 0}},
        {{4, 5}}, {{5, 6}}, {{6, 7}}, {{7, 4}},
        {{0, 4}}, {{1, 5}}, {{2, 6}}, {{3, 7}}
    }};
    ...
    for (const OctreeOverlayNode &node : nodes) {
        ...
        for (std::size_t cornerIndex = 0; cornerIndex < corners.size(); ++cornerIndex) {
            if (!projectCorner(viewport, mode, camera, zoom, corners[cornerIndex], projectedCorners[cornerIndex])) {
                allProjected = false;
                break;
            }
        }
        ...
        for (const std::array<int, 2> &edge : kCubeEdges) {
            painter.drawLine(projectedCorners[static_cast<std::size_t>(edge[0])], projectedCorners[static_cast<std::size_t>(edge[1])]);
        }
    }
