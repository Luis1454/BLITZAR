...
class MainWindowControllerLocal final {
    public:
        static void applySharedConfig(const SimulationConfig &config, grav_client::IClientRuntime &runtime)
        {
            ...
            runtime.setEnergyMeasurementConfig(config.energyMeasureEverySteps, config.energySampleLimit);
...
MainWindowApplyConfigResult MainWindowController::applyConfig(
...
std::uint32_t MainWindowController::applyPerformanceProfile(
    const SimulationConfig &config,
...
grav_config::ScenarioValidationReport MainWindowController::validate(const SimulationConfig &config) const
{
