...
void EnergyGraphWidgetPaint::paint(QWidget &widget, const std::vector<EnergyPoint> &history, UiPaintEvent *event)
{
    (void)event;
    QPainter p(&widget);
    ...
    const auto drawAxisLabels = [&]() {
        p.setPen(labelColor);
        p.drawText(QRectF(energyRect.left() + 6.0, energyRect.top() + 2.0, 120.0, 14.0), Qt::AlignLeft | Qt::AlignVCenter, EnergyGraphWidget::energyYAxisLabel());
        p.drawText(QRectF(driftRect.left() + 6.0, driftRect.top() + 2.0, 100.0, 14.0), Qt::AlignLeft | Qt::AlignVCenter, EnergyGraphWidget::driftYAxisLabel());
    };
    const auto drawLegend = [&]() {
        const std::array<QColor, 6> colors = { kineticColor, potentialColor, thermalColor, radiatedColor, totalColor, driftColor };
    ...
    const auto drawHeader = [&]() {
        p.setPen(labelColor);
        p.drawText(QRectF(headerRect.left(), headerRect.top(), headerRect.width(), headerLineHeight), Qt::AlignLeft | Qt::AlignVCenter, QStringLiteral("Energy timeline"));
        if (history.empty()) {
            p.drawText(QRectF(headerRect.left(), headerRect.top() + headerLineHeight, headerRect.width(), headerLineHeight), Qt::AlignLeft | Qt::AlignVCenter, QStringLiteral("Waiting for telemetry"));
            return;
        }
        ...
        const QString summary = QStringLiteral("Total %1    Drift %2    Time %3    Samples %4")
            ...
            .arg(history.size());
        p.drawText(QRectF(headerRect.left(), headerRect.top() + headerLineHeight, headerRect.width(), headerLineHeight), Qt::AlignLeft | Qt::AlignVCenter, summary);
    };
    ...
    std::size_t visibleStart = (history.size() > visibleSampleLimit) ? (history.size() - visibleSampleLimit) : 0u;
    const EnergyPoint &latest = history.back();
    ...
    for (std::size_t i = visibleStart; i < history.size(); ++i) {
        ...
        minEnergy = std::min(minEnergy, std::min(std::min(sample.kinetic, sample.potential), std::min(sample.thermal, std::min(sample.radiated, sample.total))));
        maxEnergy = std::max(maxEnergy, std::max(std::max(sample.kinetic, sample.potential), std::max(sample.thermal, std::max(sample.radiated, sample.total))));
    ...
    const auto buildPath = [&](auto valueAccessor, const QRectF &targetRect, float vMin, float vMax, bool centered) {
        QPainterPath path;
        for (std::size_t i = visibleStart; i < history.size(); ++i) {
            const qreal timeNorm = static_cast<qreal>((history[i].time - minTime) / (maxTime - minTime));
    ...
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.kinetic; }, energyRect, minEnergy, maxEnergy, false));
    p.setPen(QPen(faded(potentialColor), 1.2));
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.potential; }, energyRect, minEnergy, maxEnergy, false));
    p.setPen(QPen(totalColor, 2.1));
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.total; }, energyRect, minEnergy, maxEnergy, false));
    p.setPen(QPen(faded(thermalColor), 1.0));
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.thermal; }, energyRect, minEnergy, maxEnergy, false));
    p.setPen(QPen(faded(radiatedColor), 1.0));
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.radiated; }, energyRect, minEnergy, maxEnergy, false));
    ...
    p.drawPath(buildPath([](const EnergyPoint &point) { return point.drift; }, driftRect, -maxAbsDrift, maxAbsDrift, true));
    ...
    const qreal latestDriftY = driftRect.top() + driftRect.height() * (1.0 - ((latest.drift / maxAbsDrift) * 0.5f + 0.5f));
    ...
    p.drawText(QRectF(energyRect.right() - 180.0, energyRect.top() + 2.0, 176.0, 14.0), Qt::AlignRight | Qt::AlignVCenter, QStringLiteral("Current %1").arg(formatMetric(latest.total, QStringLiteral(" J"))));
    p.drawText(QRectF(driftRect.right() - 180.0, driftRect.top() + 2.0, 176.0, 14.0), Qt::AlignRight | Qt::AlignVCenter, QStringLiteral("Current %1").arg(formatMetric(latest.drift, QStringLiteral("%"))));
    p.drawText(QRectF(driftRect.left(), driftRect.top() + 2.0, 180.0, 14.0), Qt::AlignLeft | Qt::AlignVCenter, QStringLiteral("Window %1 s").arg(QString::number(maxTime - minTime, 'f', 2)));
