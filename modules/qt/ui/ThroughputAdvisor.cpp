...
public:
    ...
    {
        const std::uint32_t effectiveCap = std::max<std::uint32_t>(2u, drawCap == 0u ? config.clientParticleCap : drawCap);
        return std::max(1.0f, static_cast<float>(effectiveCap) / 50000.0f);
    }

    static std::string suggestedAction(const SimulationConfig &config, std::uint32_t substeps, std::uint32_t drawCap)
    {
...
ThroughputAdvisory ThroughputAdvisor::evaluate(const SimulationConfig &config, std::uint32_t drawCap)
{
    ...
    advisory.severity =
        advisory.estimatedStepsPerSecond < 1.0f
            ? ThroughputAdvisorySeverity::Warning
            : ThroughputAdvisorySeverity::Advisory;
    std::ostringstream summary;
    summary.setf(std::ios::fixed);
    summary.precision(1);
    summary << (advisory.severity == ThroughputAdvisorySeverity::Warning
            ? "Throughput warning"
            : "Throughput advisory")
        << ": estimated " << advisory.estimatedStepsPerSecond
        << " step/s with solver=" << config.solver
        << ", particles=" << config.particleCount
        << ", substeps=" << advisory.estimatedSubsteps
        << ", draw_cap=" << std::max<std::uint32_t>(2u, drawCap == 0u ? config.clientParticleCap : drawCap);
    advisory.summary = summary.str();
    advisory.action = ThroughputAdvisorLocal::suggestedAction(config, advisory.estimatedSubsteps, drawCap);
