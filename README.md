...
- `nightly-full`: hosted nightly build + non-GPU tests (`ctest -LE gpu`) on `windows-latest`; optional full GPU tests activate only if repo variable `HAS_GPU_RUNNER=true`.
