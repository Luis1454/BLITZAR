/*
 * @file engine/platform/process/PltProcessPosix.cpp
 * @author Luis1454
 * @project BLITZAR
 * @brief Platform abstraction implementation for portable runtime services.
 */

#include "platform/process/PltProcessOps.hpp"
#include "platform/errors/PltErrors.hpp"
#include <cerrno>
#include <signal.h>
#include <spawn.h>
#include <sys/wait.h>
                   ...
                   std::string& outError)
{
    ...
    outHandle = static_cast<NativeProcessHandle>(pid);
    return true;
}
                      ...
                      std::string& outError)
{
    ...
    while (waitedMs < waitMs) {
        ...
        usleep(sleepStrideMs * 1000u);
        waitedMs += sleepStrideMs;
    }
