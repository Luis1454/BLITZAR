/*
 * @file engine/physics/thermal/energy/ThmThermal.inl
 * @author Luis1454
 * @project BLITZAR
 ...
 */
#include "physics/thermal/model/ThmModel.inl"
...
float ParticleSystem::applyThermalModel(float deltaTime)
{
    ...
    for (Particle &p : _particles) {
        ...
        if (_thermalRadiationCoeff > 0.0f && heatCapacity > 1e-9f) {
            const float radius = cbrtf((3.0f * mass) / (4.0f * kPi * kThmParticleDensity));
            const float area = 4.0f * kPi * radius * radius;
            const float t4 = powf(temperature, 4.0f);
            const float netRadiativePower = _thermalRadiationCoeff * kThmStefanBoltzmann * area * (t4 - ambient4);
