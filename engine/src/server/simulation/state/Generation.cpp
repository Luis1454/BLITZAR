...
#pragma omp for nowait
            for (std::size_t i = 0; i < plummerData.particleCount(); ++i) {
                Particle particle;
                particle.setMass(mass);
                const Vector3 offset(plummerData.get(i, 6), plummerData.get(i, 7), plummerData.get(i, 8));
                particle.setPosition(centralPos + offset);
                particle.setVelocity(centralVel + Vector3(plummerData.get(i, 3),
                                                           plummerData.get(i, 4),
                                                           plummerData.get(i, 5)));
                finalizeParticle(particle);
...
#pragma omp parallel
        {
            std::vector<Particle> threadParticles;
#pragma omp for nowait
            for (std::size_t i = 0; i < randomValues.particleCount(); ++i) {
                Particle p;
                p.setMass(particleMass);
                p.setPosition(Vector3(centralPos.x + randomValues.get(i, 0),
                                      centralPos.y + randomValues.get(i, 1),
                                      centralPos.z + randomValues.get(i, 2)));
                p.setVelocity(Vector3(centralVel.x + randomValues.get(i, 3),
                                      centralVel.y + randomValues.get(i, 4),
                                      centralVel.z + randomValues.get(i, 5)));
                finalizeParticle(p);
...
#pragma omp parallel
    {
        std::vector<Particle> threadParticles;
#pragma omp for nowait
        for (std::size_t i = 0; i < orbitalData.particleCount(); ++i) {
            const float r = orbitalData.get(i, 0);
            const float angle = orbitalData.get(i, 1);
            const float z = orbitalData.get(i, 2);
