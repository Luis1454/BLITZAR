                         ...
                         const InitialStateConfig& config)
{
    ...
    /// Description: Executes the rng operation.
    ...
    /// Description: Executes the centralPos operation.
    const Vector3 centralPos(config.centralX, config.centralY, config.centralZ);
    /// Description: Executes the centralVel operation.
    ...
    auto applyThermalVelocity = [&](Particle& p) {
        ...
        /// Description: Executes the thermalDist operation.
    ...
    auto finalizeParticle = [&](Particle& particle) {
        /// Description: Executes the applyThermalVelocity operation.
    ...
    if (mode == "two_body") {
        ...
        /// Description: Executes the finalizeParticle operation.
    ...
    if (mode == "plummer_sphere") {
        ...
        const float sigma = std::sqrt(totalMass / std::max(6.0f * scale, 1e-6f)) *
                            /// Description: Executes the max operation.
                            std::max(0.0f, config.velocityScale);
        /// Description: Executes the unitDist operation.
        std::uniform_real_distribution<float> unitDist(1e-4f, 0.9999f);
        /// Description: Executes the azimuthDist operation.
        std::uniform_real_distribution<float> azimuthDist(0.0f, 2.0f * 3.1415926535f);
        /// Description: Executes the cosThetaDist operation.
        std::uniform_real_distribution<float> cosThetaDist(-1.0f, 1.0f);
        /// Description: Executes the velDist operation.
        std::normal_distribution<float> velDist(0.0f, sigma);
        /// Description: Executes the meanPosition operation.
        Vector3 meanPosition(0.0f, 0.0f, 0.0f);
        /// Description: Executes the meanVelocity operation.
        ...
        while (outParticles.size() < count) {
            ...
            const Vector3 offset(radius * sinTheta * std::cos(azimuth),
                                 /// Description: Executes the sin operation.
    ...
    if (mode == "random_cloud") {
        ...
        /// Description: Executes the posDist operation.
        ...
        /// Description: Executes the addCentralBody operation.
        addCentralBody();
        while (outParticles.size() < count) {
            Particle p;
    ...
    if (mode == "galaxy_collision") {
        ...
        auto generateDisk = [&](std::uint32_t n, Vector3 offset, Vector3 velocity,
            ...
            /// Description: Executes the diskRng operation.
            ...
            /// Description: Executes the rDist operation.
            std::uniform_real_distribution<float> rDist(rMin, rMax);
            /// Description: Executes the aDist operation.
            ...
            for (std::uint32_t i = 0; i < n; ++i) {
                ...
                /// Description: Executes the tangent operation.
    ...
    if (mode == "solar_system") {
        ...
        /// Description: Defines the Planet data or behavior contract.
        struct Planet {
            float r, m;
        };
        ...
        for (const auto& pInfo : planets) {
            /// Description: Executes the angleDist operation.
    ...
    if (mode == "sph_collapse" || mode == "sph_sphere") {
        ...
        while (outParticles.size() < count) {
            /// Description: Executes the dist operation.
    ...
    /// Description: Executes the radiusDist operation.
    std::uniform_real_distribution<float> radiusDist(radiusMin, radiusMax);
    /// Description: Executes the angleDist operation.
    std::uniform_real_distribution<float> angleDist(0.0f, 2.0f * 3.1415926535f);
    /// Description: Executes the zDist operation.
    ...
    while (outParticles.size() < count) {
        ...
        /// Description: Executes the radial operation.
