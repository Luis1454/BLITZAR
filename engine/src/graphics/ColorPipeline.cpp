...
static RgbColor blend(RgbColor a, RgbColor b, float t)
{
    ...
    return RgbColor{
        static_cast<std::uint8_t>(a.r + (b.r - a.r) * tt),
...
static float updateAdaptiveParameter(float current, float observed, float floorValue, float riseRate, float fallRate)
{
...
)
{
    ...
    adaptiveTemperatureScale = updateAdaptiveParameter(adaptiveTemperatureScale, observedTempMax, kTemperatureScaleFloor, 0.32f, 0.04f);
    adaptivePressureScale = updateAdaptiveParameter(adaptivePressureScale, observedPressureMax, kPressureScaleFloor, 0.32f, 0.04f);
}

ColorRGBA particleRampColorFast(const RenderParticle &particle, float temperatureScale, float pressureScale, int luminosity)
{
