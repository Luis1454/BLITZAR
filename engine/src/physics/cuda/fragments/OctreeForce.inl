Vector3 Octree::computeForceRecursive(
    ...
    if (!hasChildren(node)) {
        ...
        for (size_t i = 0; i < node.particleIndices.size(); ++i) {
            ...
            force += gravityAccelerationFromSource(particlePos, other.getPosition(), other.getMass(), softening);
        }
        return force;
    }
    ...
    if (!containsSelf && (size / distance) < theta) {
        return gravityAccelerationFromSource(particlePos, node.centerOfMass, node.mass, softening);
    }
