...
static NativeSocket toNative(std::intptr_t handle) { return static_cast<NativeSocket>(handle); }
static std::intptr_t toStored(NativeSocket handle) { return static_cast<std::intptr_t>(handle); }
...
std::intptr_t invalidNativeSocket() { return toStored(kInvalidNativeSocket); }
bool initializeSocketLayer()
{
    WSADATA wsaData{};
    return WSAStartup(MAKEWORD(2, 2), &wsaData) == 0;
}
void shutdownSocketLayer() { WSACleanup(); }
std::intptr_t createTcpSocketNative() { return toStored(::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)); }

void closeSocketNative(std::intptr_t handle)
{
    const NativeSocket socket = toNative(handle);
    if (socket != kInvalidNativeSocket) closesocket(socket);
}

bool setReuseAddressNative(std::intptr_t handle, bool enabled)
{
    const int value = enabled ? 1 : 0;
    return ::setsockopt(toNative(handle), SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&value), sizeof(value)) == 0;
}

bool setSocketTimeoutNative(std::intptr_t handle, int timeoutMs)
{
    const DWORD timeoutValue = static_cast<DWORD>(timeoutMs);
    const bool recvOk = ::setsockopt(toNative(handle), SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char *>(&timeoutValue), sizeof(timeoutValue)) == 0;
    const bool sendOk = ::setsockopt(toNative(handle), SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char *>(&timeoutValue), sizeof(timeoutValue)) == 0;
...
bool connectIpv4Native(std::intptr_t handle, const SocketAddressV4 &address, int timeoutMs)
{
    ...
    if (!setNonBlocking(socket, true)) return false;
    ...
    const int getOptResult = ::getsockopt(socket, SOL_SOCKET, SO_ERROR, reinterpret_cast<char *>(&soError), &soErrorLen);
    (void)setNonBlocking(socket, false);
...
bool listenSocketNative(std::intptr_t handle, int backlog) { return ::listen(toNative(handle), backlog) == 0; }

std::intptr_t acceptSocketNative(std::intptr_t handle)
{
    sockaddr_in clientAddr{};
    int clientAddrLen = static_cast<int>(sizeof(clientAddr));
    return toStored(::accept(toNative(handle), reinterpret_cast<sockaddr *>(&clientAddr), &clientAddrLen));
}
...
{
    return buffer.empty() ? 0 : ::recv(toNative(handle), reinterpret_cast<char *>(buffer.data), static_cast<int>(buffer.size), 0);
}

int sendBytesNative(std::intptr_t handle, grav_socket::ConstBytes buffer)
{
    return buffer.empty() ? 0 : ::send(toNative(handle), reinterpret_cast<const char *>(buffer.data), static_cast<int>(buffer.size), 0);
}
