...
bool initializeSocketLayer() { return true; }
...
void closeSocketNative(std::intptr_t handle)
{
    ...
    if (socket != kInvalidNativeSocket) close(socket);
}
...
bool waitReadableNative(std::intptr_t handle, int timeoutMs)
{
    ...
    timeout.tv_sec = timeoutMs / 1000;
    timeout.tv_usec = (timeoutMs % 1000) * 1000;
    return ::select(static_cast<int>(toNative(handle) + 1), &readSet, nullptr, nullptr, &timeout) > 0;
}

int recvBytesNative(std::intptr_t handle, grav_socket::MutableBytes buffer)
{
    return buffer.empty() ? 0 : static_cast<int>(::recv(toNative(handle), buffer.data, buffer.size, 0));
}

int sendBytesNative(std::intptr_t handle, grav_socket::ConstBytes buffer)
{
    return buffer.empty() ? 0 : static_cast<int>(::send(toNative(handle), buffer.data, buffer.size, 0));
}

bool wouldBlockOrTimeoutLastErrorNative() { return errno == EAGAIN || errno == EWOULDBLOCK; }
