...
bool DynamicLibrary::loadSymbolAddress(std::string_view name, std::uintptr_t &outSymbol, std::string &outError) const
{
...
bool DynamicLibrary::loadRawSymbol(std::string_view name, std::uintptr_t &outSymbol, std::string &outError) const
{
    try {
        ...
        if (!grav_platform_detail::loadDynamicSymbol(_impl->handle, name, outSymbol, outError)) {
            if (outError.empty()) {
                outError = std::string(grav_platform_errors::kMissingSymbolPrefix) + std::string(name);
            }
            return false;
        }
        return true;
