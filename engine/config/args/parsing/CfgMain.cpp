/*
 * @file engine/config/args/parsing/CfgMain.cpp
 * @author Luis1454
 * @project BLITZAR
...
#include "config/args/options/CfgClientOptions.hpp"
#include "config/args/options/CfgCoreOptions.hpp"
#include "config/args/options/CfgInitOptions.hpp"
#include "config/args/parsing/CfgParse.hpp"
                       ...
                       RuntimeArgs& runtime, std::ostream& warnings)
{
    ...
    for (std::size_t i = 1; i < args.size(); ++i) {
        ...
        if (applyInitOptions(key, value, config, runtime, warnings)) {
            continue;
        }
    ...
    if (!bltzr_modes::isSupportedSolverIntegratorPair(config.solver, config.integrator)) {
        if (config.solver == bltzr_modes::kSolverOctreeGpu &&
            ...
            warnings << "[args] unsupported solver/integrator combination: solver=octree_gpu does "
                        "not support rk4, using euler\n";
            config.integrator.assign(bltzr_modes::kIntegratorEuler);
        }
        else {
