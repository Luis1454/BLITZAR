/*
 * @file engine/server/src/simulation/lifecycle/Loop.cpp
 * @author Luis1454
 * @project BLITZAR
...
void SimulationServer::loop()
{
    ...
    float appliedTheta = 0.0f;
    float appliedSoftening = 0.0f;
    float appliedSphSmoothingLength = 0.0f;
    float appliedSphRestDensity = 0.0f;
    float appliedSphGasConstant = 0.0f;
    float appliedSphViscosity = 0.0f;
    std::string appliedSolverMode;
    std::string appliedIntegratorMode;
    std::string appliedOpeningCriterion;
    bool appliedSphEnabled = false;
    while (_running.load(std::memory_order_relaxed)) {
        ...
        std::string performanceProfile;
        {
            ...
            theta = _configState._octreeThetaAutoTune ? _configState._octreeEffectiveTheta : _configState._octreeTheta;
            softening = _configState._octreeSoftening;
            solverMode = _configState._solverMode;
        ...
        if (!runtimeConfigApplied || appliedSolverMode != solverMode ||
            appliedIntegratorMode != integratorMode || appliedTheta != theta ||
            appliedSoftening != softening || appliedOpeningCriterion != openingCriterion ||
            appliedSphEnabled != sphEnabled ||
            appliedSphSmoothingLength != sphSmoothingLength ||
            appliedSphRestDensity != sphRestDensity ||
            appliedSphGasConstant != sphGasConstant || appliedSphViscosity != sphViscosity) {
            ...
            _system->setSphParameters(sphSmoothingLength, sphRestDensity, sphGasConstant,
                                      sphViscosity);
            appliedSolverMode = solverMode;
            appliedIntegratorMode = integratorMode;
            appliedTheta = theta;
            appliedSoftening = softening;
            appliedOpeningCriterion = openingCriterion;
            appliedSphEnabled = sphEnabled;
            appliedSphSmoothingLength = sphSmoothingLength;
            appliedSphRestDensity = sphRestDensity;
            appliedSphGasConstant = sphGasConstant;
            appliedSphViscosity = sphViscosity;
        ...
        if (updateFailed) {
            ...
            {
                std::lock_guard<std::mutex> lock(_commandMutex);
                previousSolver = _configState._solverMode;
                if (_configState._solverMode == "pairwise_cuda" || _configState._solverMode == "octree_gpu") {
                    _configState._solverMode = "octree_cpu";
