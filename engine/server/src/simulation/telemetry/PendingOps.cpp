/*
 * @file engine/server/src/simulation/telemetry/PendingOps.cpp
 * @author Luis1454
 * @project BLITZAR
...
void SimulationServer::processPendingExport()
{
    ...
    std::string exportDirectory;
    {
        ...
        exportDirectory = _configState._exportDirectory;
    }
    if (!hasRequest) {
        return;
    }
    outputPath = request.outputPath;
    format = request.format.empty() ? _configState._exportFormatDefault : request.format;
    if (outputPath.empty()) {
        outputPath =
...
bool SimulationServer::exportCurrentState(const std::string& outputPath, const std::string& format)
{
    ...
    {
        std::lock_guard<std::mutex> lock(_commandMutex);
        solverModeLabel = _configState._solverMode;
        integratorModeLabel = _configState._integratorMode;
    }
