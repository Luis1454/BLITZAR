/*
 * @file engine/server/simulation/runtime/SrvHelpers.cpp
 * @author Luis1454
 * @project BLITZAR
...
std::string executableSolverMode(std::string solver)
{
    ...
    if (solver == bltzr_modes::kSolverOctreeGpu) {
        return std::string(bltzr_modes::kSolverOctreeCpu);
    }
    return solver;
#endif
}
...
{
    if (!std::isfinite(dt))
        return kMinSimulationDt;
    return std::clamp(dt, kMinSimulationDt, kMaxStableInteractiveDt);
}
...
{
    return std::clamp(value, kPhysicsMinTheta, kPhysicsMaxTheta);
}
