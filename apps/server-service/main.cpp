...
int main(int argc, char **argv)
{
    ...
    if (runtime.hasArgumentError) {
        grav_server_service::printServerHelp(options.simArgs.empty() ? std::string_view("myAppServer") : options.simArgs[0]);
        return 2;
    }
    ...
    if (!daemon.start(options.port, options.host)) {
        std::cerr << "[server] failed to start IPC server on " << options.host << ":" << options.port << "\n";
    ...
    while (!grav_server_service::stopRequested() && !daemon.shutdownRequested()) {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - lastLog).count() >= 2) {
            ...
            std::cout << "[server] step=" << stats.steps
