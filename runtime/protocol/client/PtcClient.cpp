/*
 * @file runtime/protocol/client/PtcClient.cpp
 * @author Luis1454
 * @project BLITZAR
 ...
 * @return Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
Client::Client()
    /*
 ...
 * @return bool Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
 ...
 * @param timeoutMs Input value used by this contract.
 * @return void Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
void Client::setSocketTimeoutMs(int timeoutMs)
{
 ...
 * @param None This contract does not take explicit parameters.
 * @return int Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
int Client::socketTimeoutMs() const
{
 ...
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
void Client::setAuthToken(std::string token)
{
 ...
 * @return std::string Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
std::string Client::trim(const std::string& value)
{
 ...
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
bool Client::readLine(std::string& outLine)
{
    try {
 ...
 * @return Response Client:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
Response Client::sendJson(const std::string& jsonLine)
{
    Response response;
    try {
        if (!isConnected()) {
        ...
        if (!bltzr_protocol::JsonCodec::parseResponseEnvelope(response.raw, envelope,
                                                                    response.error)) {
 ...
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
Response Client::sendCommand(const std::string& cmd,
                                               const std::string& fieldsJson)
{
    bltzr_protocol::CommandRequest request{};
    request.cmd = cmd;
    request.token = _authToken;
    return sendJson(bltzr_protocol::JsonCodec::makeRequest(request, fieldsJson));
}
 ...
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
Response Client::getStatus(ClientStatus& outStatus)
{
    try {
        Response response = sendCommand(std::string(bltzr_protocol::Status));
        if (!response.ok)
            return response;
        bltzr_protocol::StatusPayload parsed{};
        std::string parseError;
        if (!bltzr_protocol::JsonCodec::parseStatusResponse(response.raw, parsed,
                                                                  parseError)) {
            response.ok = false;
                                               ...
                                               std::uint32_t maxPoints, std::size_t* outSourceSize)
{
    try {
        ...
        Response response = sendCommand(std::string(bltzr_protocol::GetSnapshot),
                                                    "\"max_points\":" + std::to_string(maxPoints));
        if (!response.ok)
            return response;
        bltzr_protocol::SnapshotPayload parsed{};
        std::string parseError;
        if (!bltzr_protocol::JsonCodec::parseSnapshotResponse(response.raw, parsed,
                                                                    parseError)) {
            response.ok = false;
