/*
 * @file runtime/protocol/codec/parser/PtcParser.cpp
 * @author Luis1454
 * @project BLITZAR
...
bool JsonCodec::parseRequest(std::string_view raw, CommandRequest& out,
                                          std::string& error)
{
...
bool JsonCodec::parseResponseEnvelope(std::string_view raw, ResponseEnvelope& out,
                                            std::string& error)
{
    const std::string trimmedRaw = trim(raw);
    if (trimmedRaw.empty() || trimmedRaw.front() != '{' || trimmedRaw.back() != '}') {
...
bool JsonCodec::readString(std::string_view raw, std::string_view key, std::string& out)
{
    std::size_t cursor = 0;
...
bool JsonCodec::readBool(std::string_view raw, std::string_view key, bool& out)
{
    std::string token;
...
std::string JsonCodec::trim(std::string_view value)
{
...
std::string JsonCodec::toLower(std::string value)
{
...
bool JsonCodec::findValueStart(std::string_view raw, std::string_view key, std::size_t& start)
{
...
bool JsonCodec::readToken(std::string_view raw, std::string_view key, std::string& out)
{
    std::size_t cursor = 0;
