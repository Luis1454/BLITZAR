/*
 * @file runtime/protocol/codec/PtcJsonCodec.cpp
 * @author Luis1454
 * @project BLITZAR
...
public:
    ...
    {
        writeKey(key);
        _out << "\"" << JsonCodec::escapeString(value) << "\"";
    }
...
std::string JsonCodec::escapeString(std::string_view value)
{
    std::string escaped;
...
std::string JsonCodec::makeRequest(const CommandRequest& request,
...
std::string JsonCodec::makeOkResponse(std::string_view cmd)
{
...
std::string JsonCodec::makeErrorResponse(std::string_view cmd, std::string_view message)
{
    std::ostringstream out;
    ServerJsonObjectWriter writer(out);
...
std::string JsonCodec::makeSnapshotResponse(bool hasSnapshot,
