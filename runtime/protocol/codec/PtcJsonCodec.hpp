/*
 * @file runtime/protocol/codec/PtcJsonCodec.hpp
 * @author Luis1454
 * @project BLITZAR
...
public:
    ...
    static std::string makeRequest(const CommandRequest& request,
                                          std::string_view fieldsJson = {});
    static bool parseRequest(std::string_view raw, CommandRequest& out,
                                    std::string& error);
    ...
    static std::string makeSnapshotResponse(bool hasSnapshot,
                                            ...
                                            std::size_t sourceSize = 0u);
    static bool parseResponseEnvelope(std::string_view raw, ResponseEnvelope& out,
                                      std::string& error);
    static bool parseStatusResponse(std::string_view raw, StatusPayload& out,
                                    std::string& error);
    static bool parseSnapshotResponse(std::string_view raw, SnapshotPayload& out,
                                      std::string& error);
