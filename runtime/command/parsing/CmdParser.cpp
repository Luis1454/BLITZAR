/*
 * @file runtime/command/parsing/CmdParser.cpp
 * @author Luis1454
 * @project BLITZAR
...
static ParseResult parseTokens(const std::vector<std::string>& tokens,
                                      std::size_t lineNumber)
{
    ...
    for (std::size_t index = 0u; index < provided; ++index) {
        ...
        if (argument.kind == ArgumentKind::Float) {
            double value = 0.0;
...
ParseResult parseScript(const std::string& scriptText)
{
    ...
    for (char c : scriptText) {
        if (c == '\n') {
            ParseResult lineResult = parseLine(current, lineNumber);
            if (!lineResult.ok) {
                return lineResult;
            }
    ...
    ParseResult tailResult = parseLine(current, lineNumber);
    if (!tailResult.ok) {
        return tailResult;
    }
...
ParseResult parseLine(const std::string& line, std::size_t lineNumber)
{
    ...
    // Batch scripts may include interactive exit keywords; accept them as no-op.
