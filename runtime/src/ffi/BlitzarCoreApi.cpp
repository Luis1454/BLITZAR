...
{
    return blitzar_core_config_t{10000u, 0.01f, "pairwise_cuda", "euler", "interactive", 0.01f, 4u, 50u};
}

blitzar_core_t *blitzar_core_create(
    const blitzar_core_config_t *config,
    char *error_buffer,
    size_t error_buffer_capacity)
{
    if (config == nullptr) {
        grav_ffi_internal::writeErrorMessage("config is required", error_buffer, error_buffer_capacity);
        return nullptr;
    }
    try {
        ...
        grav_ffi_internal::writeErrorMessage("allocation failed", error_buffer, error_buffer_capacity);
        return nullptr;
    } catch (...) {
        grav_ffi_internal::writeErrorMessage("unexpected exception during core creation", error_buffer, error_buffer_capacity);
        return nullptr;
    }
}
...
blitzar_core_result_t blitzar_core_apply_config(blitzar_core_t *core, const blitzar_core_config_t *config)
{
...
blitzar_core_result_t blitzar_core_run_steps(blitzar_core_t *core, uint32_t steps, uint32_t timeout_ms)
{
    if (grav_ffi_internal::invalidArgumentIfNull(core) != BLITZAR_CORE_OK) {
        return BLITZAR_CORE_INVALID_ARGUMENT;
    }
...
blitzar_core_result_t blitzar_core_get_status(const blitzar_core_t *core, blitzar_core_status_t *out_status)
{
    if (grav_ffi_internal::invalidArgumentIfNull(core) != BLITZAR_CORE_OK
...
blitzar_core_result_t blitzar_core_get_snapshot(
    const blitzar_core_t *core,
    size_t max_points,
...
blitzar_core_result_t blitzar_core_load_state(
    blitzar_core_t *core,
    const char *path,
    const char *format,
    uint32_t timeout_ms)
...
blitzar_core_result_t blitzar_core_export_state(
    blitzar_core_t *core,
    const char *path,
    const char *format,
    uint32_t timeout_ms)
...
size_t blitzar_core_get_last_error(const blitzar_core_t *core, char *error_buffer, size_t error_buffer_capacity)
{
    if (core == nullptr) {
        grav_ffi_internal::writeErrorMessage("core is required", error_buffer, error_buffer_capacity);
