...
blitzar_core_result_t BlitzarCore::waitForSnapshot(std::uint32_t timeoutMs) const
{
    ...
    if (return _server.copyLatestSnapshot(snapshot, 0u);)))
        {
            return BLITZAR_CORE_OK;
        }
...
{
    std::vector<RenderParticle> snapshot;
    if (pollUntil(timeoutMs, [this, &config, &
                              snapshot]() return hasMatchingConfig(_server.getStats(), config) &&
...
blitzar_core_result_t BlitzarCore::waitForFile(const char* path, std::uint32_t timeoutMs) const
{
    ...
    if (pollUntil(timeoutMs, [fsPath]() return std::filesystem::exists(fsPath) &&
