...
class BlitzarCore final {
    public:
        ...
        blitzar_core_result_t getSnapshot(std::size_t maxPoints, blitzar_core_snapshot_t &outSnapshot) const;
        blitzar_core_result_t loadState(const char *path, const char *format, std::uint32_t timeoutMs);
        blitzar_core_result_t exportState(const char *path, const char *format, std::uint32_t timeoutMs);
    ...
    private:
        ...
        blitzar_core_result_t waitForAppliedConfig(const blitzar_core_config_t &config, std::uint32_t timeoutMs) const;
        blitzar_core_result_t waitForStepTarget(std::uint64_t expectedSteps, std::uint32_t timeoutMs) const;
