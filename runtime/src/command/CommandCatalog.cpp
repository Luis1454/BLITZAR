...
const std::vector<CommandSpec> &CommandCatalog::all()
{
    static const std::vector<CommandSpec> specs = {
        ...
        makeSpec(CommandId::LoadConfig, "load_config", "load config file and apply it", true, {{"path", CommandArgumentKind::Path, false, {}}}),
        makeSpec(CommandId::Connect, "connect", "connect to remote server", true, {{"host", CommandArgumentKind::Host, false, {}}, {"port", CommandArgumentKind::Port, false, {}}}),
        ...
        makeSpec(CommandId::Step, "step", "advance one or more steps", true, {{"count", CommandArgumentKind::Uint, true, {}}}),
        ...
        makeSpec(CommandId::SetDt, "set_dt", "set simulation dt in seconds", true, {{"seconds", CommandArgumentKind::Float, false, {}}}),
        makeSpec(CommandId::SetSolver, "set_solver", "set solver mode", true, {{"name", CommandArgumentKind::Enum, false, {"pairwise_cuda", "octree_gpu", "octree_cpu"}}}),
        makeSpec(CommandId::SetIntegrator, "set_integrator", "set integrator mode", true, {{"name", CommandArgumentKind::Enum, false, {"euler", "rk4"}}}),
...
std::string CommandCatalog::renderHelp()
{
    ...
    for (const CommandSpec &spec : all()) {
        ...
        for (const CommandArgumentSpec &argument : spec.arguments) {
            out << " " << (argument.optional ? "[" : "<") << argument.name << (argument.optional ? "]" : ">");
        }
        out << " : " << spec.help << "\n";
    }
    return out.str();
