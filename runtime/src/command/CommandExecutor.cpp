...
static CommandResult applyConfig(CommandExecutionContext& context, bool requestReset)
{
    ...
    result = sendChecked(
        ...
        "\"value\":\"" +
            bltzr_protocol::ServerJsonCodec::escapeString(context.session.config.solver) + "\"");
    if (!result.ok)
        return result;
    result = sendChecked(
        context, std::string(bltzr_protocol::SetIntegrator),
        "\"value\":\"" +
            bltzr_protocol::ServerJsonCodec::escapeString(context.session.config.integrator) +
            "\"");
    if (!result.ok)
        return result;
    result = sendChecked(context, std::string(bltzr_protocol::SetPerformanceProfile),
                         "\"value\":\"" +
                             bltzr_protocol::ServerJsonCodec::escapeString(
    ...
    if (!initPlan.inputFile.empty()) {
        result = sendChecked(context, std::string(bltzr_protocol::Load),
                             "\"path\":\"" +
                                 bltzr_protocol::ServerJsonCodec::escapeString(initPlan.inputFile) +
                                 "\",\"format\":\"" +
                                 bltzr_protocol::ServerJsonCodec::escapeString(
...
{
    CommandResult result = sendChecked(context, std::string(bltzr_protocol::Pause));
    if (!result.ok)
        return result;
                                       ...
                                       CommandExecutionContext& context)
{
    ...
    if (request.id == CommandId::SetSolver) {
        ...
        return sendChecked(context, std::string(bltzr_protocol::SetSolver),
                           "\"value\":\"" +
                               bltzr_protocol::ServerJsonCodec::escapeString(canonical) + "\"");
    }
    ...
    if (request.id == CommandId::ExportSnapshot) {
        ...
        return sendChecked(
            ...
            "\"path\":\"" + bltzr_protocol::ServerJsonCodec::escapeString(path) +
                "\",\"format\":\"" +
                bltzr_protocol::ServerJsonCodec::escapeString(bltzr_client::normalizeExportFormat(
    ...
    if (request.id == CommandId::SaveCheckpoint)
        return sendChecked(
            ...
            "\"path\":\"" + bltzr_protocol::ServerJsonCodec::escapeString(argString(request, 0u)) +
                "\"");
