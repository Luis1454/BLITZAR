...
static CommandResult sendChecked(CommandExecutionContext &context, const std::string &cmd, const std::string &fields = {})
{
    if (!context.transport.isConnected() && !context.transport.connect(context.session.host, context.session.port)) {
        return makeFailure("unable to connect to server " + context.session.host + ":" + std::to_string(context.session.port));
    }
...
{
    const grav_config::ScenarioValidationReport report = grav_config::SimulationScenarioValidation::evaluate(context.session.config);
    if (!report.validForRun) {
        return makeFailure(grav_config::SimulationScenarioValidation::renderText(report));
    }
    CommandResult result = sendChecked(context, std::string(grav_protocol::SetParticleCount), "\"value\":" + std::to_string(grav_client::resolveServerParticleCount(context.session.config)));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetDt), "\"value\":" + std::to_string(context.session.config.dt));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetSolver), "\"value\":\"" + grav_protocol::ServerJsonCodec::escapeString(context.session.config.solver) + "\"");
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetIntegrator), "\"value\":\"" + grav_protocol::ServerJsonCodec::escapeString(context.session.config.integrator) + "\"");
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetPerformanceProfile), "\"value\":\"" + grav_protocol::ServerJsonCodec::escapeString(context.session.config.performanceProfile) + "\"");
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetOctree), "\"theta\":" + std::to_string(context.session.config.octreeTheta) + ",\"softening\":" + std::to_string(context.session.config.octreeSoftening));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetSph), std::string("\"value\":") + (context.session.config.sphEnabled ? "true" : "false"));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetSphParams), "\"h\":" + std::to_string(context.session.config.sphSmoothingLength) + ",\"rest_density\":" + std::to_string(context.session.config.sphRestDensity) + ",\"gas_constant\":" + std::to_string(context.session.config.sphGasConstant) + ",\"viscosity\":" + std::to_string(context.session.config.sphViscosity));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetSubsteps), "\"target_dt\":" + std::to_string(context.session.config.substepTargetDt) + ",\"max_substeps\":" + std::to_string(context.session.config.maxSubsteps));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetSnapshotPublishCadence), "\"period_ms\":" + std::to_string(context.session.config.snapshotPublishPeriodMs));
    if (!result.ok) {
        return result;
    }
    result = sendChecked(context, std::string(grav_protocol::SetEnergyMeasure), "\"every_steps\":" + std::to_string(context.session.config.energyMeasureEverySteps) + ",\"sample_limit\":" + std::to_string(context.session.config.energySampleLimit));
    if (!result.ok) {
        return result;
    }
    ...
    if (!initPlan.inputFile.empty()) {
        result = sendChecked(
            ...
            "\"path\":\"" + grav_protocol::ServerJsonCodec::escapeString(initPlan.inputFile)
                + "\",\"format\":\"" + grav_protocol::ServerJsonCodec::escapeString(initPlan.inputFormat.empty() ? "auto" : initPlan.inputFormat) + "\"");
        if (!result.ok) {
            return result;
        }
...
static CommandResult runSteps(CommandExecutionContext &context, std::uint64_t stepCount)
{
    ...
    return sendChecked(context, std::string(grav_protocol::Step), "\"count\":" + std::to_string(stepCount));
}
...
static CommandResult runUntil(CommandExecutionContext &context, double targetTime)
{
    ...
    for (;;) {
        ...
        if (!statusResponse.ok) {
            return makeFailure(statusResponse.error.empty() ? "status failed" : statusResponse.error);
        }
...
CommandResult CommandExecutor::execute(const CommandRequest &request, CommandExecutionContext &context)
{
    ...
    if (request.id == CommandId::Step) {
        return sendChecked(context, std::string(grav_protocol::Step), "\"count\":" + std::to_string(request.arguments.empty() ? 1u : argUint(request, 0u)));
    }
    ...
    if (request.id == CommandId::SetDt) {
        ...
        return sendChecked(context, std::string(grav_protocol::SetDt), "\"value\":" + std::to_string(context.session.config.dt));
    }
    if (request.id == CommandId::SetSolver) {
        ...
        return sendChecked(context, std::string(grav_protocol::SetSolver), "\"value\":\"" + grav_protocol::ServerJsonCodec::escapeString(canonical) + "\"");
    }
    if (request.id == CommandId::SetIntegrator) {
        ...
        return sendChecked(context, std::string(grav_protocol::SetIntegrator), "\"value\":\"" + grav_protocol::ServerJsonCodec::escapeString(canonical) + "\"");
    }
    ...
    if (request.id == CommandId::SetParticleCount) {
        ...
        return sendChecked(context, std::string(grav_protocol::SetParticleCount), "\"value\":" + std::to_string(context.session.config.particleCount));
    }
    if (request.id == CommandId::ExportSnapshot) {
        const std::string path = argString(request, 0u);
        const std::string format = request.arguments.size() >= 2u ? argString(request, 1u) : grav_client::inferExportFormatFromPath(path);
        return sendChecked(
            context,
            std::string(grav_protocol::Export),
