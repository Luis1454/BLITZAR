...
public:
    static bool computeFileSha256Hex(
...
private:
    ...
    static void transformBlock(std::array<std::uint32_t, 8u> &hash, const std::uint8_t *block)
    {
        ...
        for (std::size_t i = 16u; i < words.size(); ++i) {
            words[i] = smallSigma1(words[i - 2u]) + words[i - 7u] + smallSigma0(words[i - 15u]) + words[i - 16u];
        }
...
bool ClientModuleHash::computeFileSha256Hex(
    std::string_view filePath,
    std::string &outHexDigest,
    std::string &outError)
{
