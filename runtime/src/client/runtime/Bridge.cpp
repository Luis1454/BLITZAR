...
Bridge::Bridge(const std::string& configPath, std::string remoteHost,
    ...
    : _configPath(configPath),
      ...
      _remoteCommandTimeoutMs(clampRemoteTimeoutMs(remoteCommandTimeoutMs)),
      _remoteStatusTimeoutMs(clampRemoteTimeoutMs(remoteStatusTimeoutMs)),
      _remoteSnapshotTimeoutMs(clampRemoteTimeoutMs(remoteSnapshotTimeoutMs))
{
...
bool Bridge::start()
{
    ...
    const bool autoStartCapable = shouldAutoStartRemoteServer();
...
{
    std::lock_guard<std::recursive_mutex> lock(_mutex);
    const float safeTheta = std::max(kPhysicsMinTheta, theta);
    const float safeSoftening = std::max(kMinSimulationDt, softening);
...
void Bridge::setSphParameters(float smoothingLength, float restDensity,
...
void Bridge::requestExportSnapshot(const std::string& outputPath,
...
void Bridge::configureRemoteConnector(const std::string& host, std::uint16_t port,
                                                  bool autoStart,
                                                  const std::string& serverExecutable)
{
    ...
    _remotePort = (port == 0u) ? kDefaultServerPort : port;
...
SimulationStats Bridge::fromRemoteStatus(const bltzr_protocol::ClientStatus& status)
{
...
bool Bridge::sendRemoteNow(const std::string& cmd, const std::string& fields)
{
    if (!ensureRemoteConnected(false)) {
        return false;
    }
    SocketTimeoutScope timeoutScope(_remoteClient, static_cast<int>(_remoteCommandTimeoutMs));
    const bltzr_protocol::Response response = _remoteClient.sendCommand(cmd, fields);
    if (!response.ok) {
        if (isTransportClientFailure(response.error)) {
...
bool Bridge::sendOrQueueRemote(const std::string& cmd, const std::string& fields)
{
    if (!_runtimeState.isConnected() || !_remoteClient.isConnected()) {
...
void Bridge::queuePendingRemoteCommand(const std::string& cmd,
...
bool Bridge::ensureRemoteConnected(bool forceLog)
{
    if (_runtimeState.isConnected() && !_remoteClient.isConnected()) {
        _runtimeState.setConnected(false);
    }
...
void Bridge::markRemoteDisconnected(const std::string& context,
...
bool Bridge::isLoopbackHost(std::string_view host)
{
    if (host.empty()) {
        return true;
    }
    return host == kDefaultLoopbackHost || host == "localhost";
}

bool Bridge::shouldAutoStartRemoteServer() const
{
    return _remoteAutoStart && !_remoteLaunchAttempted && isLoopbackHost(_remoteHost);
}

void Bridge::tryAutoStartRemoteServer()
{
...
void Bridge::flushPendingRemoteCommands()
{
...
void Bridge::refreshRemoteStats()
{
    if (!ensureRemoteConnected(false)) {
        return;
    }
    ...
    const bltzr_protocol::Response response = _remoteClient.getStatus(status);
    if (!response.ok) {
        if (isTransportClientFailure(response.error)) {
