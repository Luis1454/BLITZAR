...
static bool extractSnapshotParticles(const std::string &raw, std::vector<RenderParticle> &outSnapshot)
{
    outSnapshot.clear();
    ...
    auto parseFloat = [&](float &out) -> bool {
        ...
        return grav_text::parseNumber(std::string_view(raw.data() + start, i - start), out);
    };
