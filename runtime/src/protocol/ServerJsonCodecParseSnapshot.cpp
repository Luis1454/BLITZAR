...
class SnapshotArrayParser {
    ...
    private:
        ...
        void skipSpaces()
        {
            while (m_cursor < m_raw.size() && std::isspace(static_cast<unsigned char>(m_raw[m_cursor])) != 0) {
                ++m_cursor;
            }
        }
...
bool ServerJsonCodec::parseSnapshotResponse(std::string_view raw, ServerSnapshotPayload &out, std::string &error)
{
