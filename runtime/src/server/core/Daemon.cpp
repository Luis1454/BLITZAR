 ...
 * @return Daemon:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
 ...
 * @return bool Daemon:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
 ...
 * @param None This contract does not take explicit parameters.
 * @return void Daemon:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
void Daemon::stop()
{
 ...
 * @return std::string Daemon:: value produced by this contract.
 * @note Keep side effects explicit and preserve deterministic behavior where callers depend on it.
 */
std::string Daemon::processRequest(const std::string& request)
{
    try {
        ...
        if (cmd == bltzr_protocol::GetSnapshot) {
            ...
            return bltzr_protocol::JsonCodec::makeSnapshotResponse(hasSnapshot, snapshot,
                                                                         sourceSize);
        }
        ...
        if (cmd == bltzr_protocol::SetDt) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "value", dt) || dt <= 0.0) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd, "invalid dt value");
            }
        ...
        if (cmd == bltzr_protocol::SetSolver) {
            ...
            if (!bltzr_protocol::JsonCodec::readString(request, "value", value)) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd,
        ...
        if (cmd == bltzr_protocol::SetParticleCount) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "value", value) ||
                value < 2ull) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd,
        ...
        if (cmd == bltzr_protocol::SetOctree) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "theta", theta) ||
                theta <= 0.0) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd, "invalid theta");
            }
            if (!bltzr_protocol::JsonCodec::readNumber(request, "softening", softening) ||
                softening <= 0.0) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd, "invalid softening");
            }
        ...
        if (cmd == bltzr_protocol::SetSphParams) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "h", h) || h <= 0.0 ||
                !bltzr_protocol::JsonCodec::readNumber(request, "rest_density",
                                                             restDensity) ||
                restDensity <= 0.0 ||
                !bltzr_protocol::JsonCodec::readNumber(request, "gas_constant",
                                                             gasConstant) ||
                gasConstant <= 0.0 ||
        ...
        if (cmd == bltzr_protocol::SetSubsteps) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "target_dt", targetDt) ||
                targetDt < 0.0 ||
                !bltzr_protocol::JsonCodec::readNumber(request, "max_substeps",
                                                             maxSubsteps) ||
                maxSubsteps < 1u) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd,
        ...
        if (cmd == bltzr_protocol::SetEnergyMeasure) {
            ...
            if (!bltzr_protocol::JsonCodec::readNumber(request, "every_steps", everySteps) ||
                everySteps < 1u ||
                !bltzr_protocol::JsonCodec::readNumber(request, "sample_limit",
                                                             sampleLimit) ||
                sampleLimit < 2u) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(
        ...
        if (cmd == bltzr_protocol::Load) {
            std::string path;
            if (!bltzr_protocol::JsonCodec::readString(request, "path", path) ||
                path.empty()) {
                return bltzr_protocol::JsonCodec::makeErrorResponse(cmd, "missing path");
            }
        ...
        return bltzr_protocol::JsonCodec::makeErrorResponse(cmd, "unknown command");
    }
    catch (const std::exception& ex) {
        return bltzr_protocol::JsonCodec::makeErrorResponse(
