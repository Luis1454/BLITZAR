/*
 * @file runtime/client/module/CliManifest.cpp
 * @author Luis1454
 * @project BLITZAR
...
bool parseLine(std::string_view line, std::string& outKey, std::string& outValue)
    {
...
bool readUnsigned(std::string_view rawValue, std::uint32_t& outValue)
    {
...
bool isHexDigest(std::string_view rawValue) noexcept
    {
                                ...
                                std::string& outError)
{
    ...
    while (std::getline(input, line)) {
        ...
        if (key == "format_version") {
            if (!readUnsigned(value, outManifest.m_formatVersion)) {
        ...
        if (key == "api_version") {
            if (!readUnsigned(value, outManifest.m_apiVersion)) {
    ...
    if (!isHexDigest(outManifest.m_sha256)) {
...
bool Manifest::validateForLoad(std::string_view modulePath,
                                           std::string_view expectedModuleId,
                                           std::string& outError) const
{
    if (!isSupportedModuleId(m_moduleId)) {
    ...
    if (m_apiVersion != kApiVersionV1) {
        outError = "unsupported module api version";
        return false;
    }
    if (m_productName != kProductName) {
        outError = "module manifest product mismatch";
        return false;
    }
    if (m_productVersion != kProductVersion) {
...
std::string_view Manifest::moduleId() const noexcept
{
    return m_moduleId;
}

std::string_view Manifest::moduleName() const noexcept
{
    return m_moduleName;
}

std::string_view Manifest::sha256() const noexcept
{
    return m_sha256;
}

std::uint32_t Manifest::apiVersion() const noexcept
{
    return m_apiVersion;
}
} // namespace bltzr_module
