/*
 * @file runtime/client/module/CliHash.cpp
 * @author Luis1454
 * @project BLITZAR
...
std::uint32_t rotateRight(std::uint32_t value, std::uint32_t shift) noexcept
{
    return (value >> shift) | (value << (32u - shift));
}

std::uint32_t choose(std::uint32_t x, std::uint32_t y, std::uint32_t z) noexcept
{
    return (x & y) ^ (~x & z);
}

std::uint32_t majority(std::uint32_t x, std::uint32_t y, std::uint32_t z) noexcept
{
    return (x & y) ^ (x & z) ^ (y & z);
}

std::uint32_t bigSigma0(std::uint32_t value) noexcept
{
    return rotateRight(value, 2u) ^ rotateRight(value, 13u) ^ rotateRight(value, 22u);
}

std::uint32_t bigSigma1(std::uint32_t value) noexcept
{
    return rotateRight(value, 6u) ^ rotateRight(value, 11u) ^ rotateRight(value, 25u);
}

std::uint32_t smallSigma0(std::uint32_t value) noexcept
{
    return rotateRight(value, 7u) ^ rotateRight(value, 18u) ^ (value >> 3u);
}

std::uint32_t smallSigma1(std::uint32_t value) noexcept
{
    return rotateRight(value, 17u) ^ rotateRight(value, 19u) ^ (value >> 10u);
}

std::vector<std::uint8_t> pad(const std::vector<std::uint8_t>& input)
{
...
void transformBlock(std::array<std::uint32_t, 8u>& hash, const std::uint8_t* block)
{
